...
| [architecture/io.md](architecture/io.md) | IO drivers and managers: the no-singleton-driver model, UI-vs-live driver split, the spec-0034 async task trees (`app/src/Async/` + `ConnectionFlows`), connection diagnostics (spec 0035), file transmission protocols. New drivers: `ss-new-driver` skill + `BluetoothLE` reference. |
