...
| Per-driver reconnect interval, retry counter, or attempt loop | Retry/backoff is declared **once** in `RetryPolicy::initialConnect()` / `autoReconnect()` and run by the spec-0034 task trees at connection boundaries — nothing per frame, no new thread, and retry attempts emit no connection-state signal. A driver opts in via the `HAL_Driver` async-open hooks, never its own timer. |
