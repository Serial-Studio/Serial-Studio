...
| **Historian**       | `--session-export`    | Enables Historian recording to the configured `.db` location. |
...
Runtime mode does **not** change anything about the data pipeline, frame parsers, exports, or the API server. The dashboard, MCP/JSON-RPC, gRPC, MQTT, and Historian all behave exactly as they would in a normal Serial Studio session. Only the surrounding chrome is changed.
...
| `--session-export`  | Pre-arm Historian recording (Pro). |
...
- [CSV Export & Playback](CSV-Export-Playback.md), [Historian](Session-Database.md): the recorders the Logging tab pre-arms.
