...
#define sk_X509_LOOKUP_new_reserve(cmp, n)                                                         \
    ((STACK_OF(X509_LOOKUP) *)OPENSSL_sk_new_reserve(ossl_check_X509_LOOKUP_compfunc_type(cmp),    \
                                                     (n)))
...
#define sk_X509_LOOKUP_deep_copy(sk, copyfunc, freefunc)                                           \
    ((STACK_OF(X509_LOOKUP) *)OPENSSL_sk_deep_copy(                                                \
        ossl_check_const_X509_LOOKUP_sk_type(sk), ossl_check_X509_LOOKUP_copyfunc_type(copyfunc),  \
        ossl_check_X509_LOOKUP_freefunc_type(freefunc)))
...
#define sk_X509_OBJECT_new_reserve(cmp, n)                                                         \
    ((STACK_OF(X509_OBJECT) *)OPENSSL_sk_new_reserve(ossl_check_X509_OBJECT_compfunc_type(cmp),    \
                                                     (n)))
...
#define sk_X509_OBJECT_deep_copy(sk, copyfunc, freefunc)                                           \
    ((STACK_OF(X509_OBJECT) *)OPENSSL_sk_deep_copy(                                                \
        ossl_check_const_X509_OBJECT_sk_type(sk), ossl_check_X509_OBJECT_copyfunc_type(copyfunc),  \
        ossl_check_X509_OBJECT_freefunc_type(freefunc)))
...
STACK_OF(X509)
*X509_build_chain(X509 *target, STACK_OF(X509) * certs, X509_STORE *store, int with_self_signed,
...
typedef STACK_OF(X509_CRL)
    * (*X509_STORE_CTX_lookup_crls_fn)(const X509_STORE_CTX *ctx, const X509_NAME *nm);
...
typedef int (*X509_LOOKUP_ctrl_fn)(X509_LOOKUP *ctx, int cmd, const char *argc, long argl,
                                   char **ret);
...
int X509_STORE_load_file_ex(X509_STORE *xs, const char *file, OSSL_LIB_CTX *libctx,
                            const char *propq);
int X509_STORE_load_store_ex(X509_STORE *xs, const char *store, OSSL_LIB_CTX *libctx,
                             const char *propq);
int X509_STORE_load_locations_ex(X509_STORE *xs, const char *file, const char *dir,
                                 OSSL_LIB_CTX *libctx, const char *propq);
