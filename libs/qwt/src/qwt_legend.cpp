  ...
  : QwtAbstractLegend(parent)
{
  ...
  QwtDynGridLayout *gridLayout
      = new QwtDynGridLayout(m_data->view->contentsWidget);
 ...
 */
void QwtLegend::renderLegend(QPainter *painter, const QRectF &rect,
 ...
 */
void QwtLegend::renderItem(QPainter *painter, const QWidget *widget,
                           const QRectF &rect, bool fillBackground) const
{
  ...
  if (label)
  {
    ...
    const QRectF iconRect(rect.x() + label->margin(),
                          rect.center().y() - 0.5 * sz.height(), sz.width(),
                          sz.height());
