...
static QPainterPath qwtCanvasClip(const QWidget *canvas,
 ...
 */
void QwtPlotRenderer::renderDocument(QwtPlot *plot, const QString &fileName,
 ...
 */
void QwtPlotRenderer::render(QwtPlot *plot, QPainter *painter,
                             const QRectF &plotRect) const
{
  ...
  if (!(m_data->discardFlags & DiscardLegend) && plot->legend()
      && !plot->legend()->isEmpty())
  {
  ...
  {
    {
      ...
      if (scaleWidget)
      {
        ...
        renderScale(plot, painter, axisId, startDist, endDist, baseDist,
 ...
 */
void QwtPlotRenderer::renderTitle(const QwtPlot *plot, QPainter *painter,
                                  const QRectF &titleRect) const
{
  painter->setFont(qwtResolvedFont(plot->titleLabel()));

  const QColor color
      = plot->titleLabel()->palette().color(QPalette::Active, QPalette::Text);
 ...
 */
void QwtPlotRenderer::renderFooter(const QwtPlot *plot, QPainter *painter,
                                   const QRectF &footerRect) const
{
  painter->setFont(qwtResolvedFont(plot->footerLabel()));

  const QColor color
      = plot->footerLabel()->palette().color(QPalette::Active, QPalette::Text);
 ...
 */
void QwtPlotRenderer::renderLegend(const QwtPlot *plot, QPainter *painter,
 ...
 */
void QwtPlotRenderer::renderCanvas(const QwtPlot *plot, QPainter *painter,
                                   const QRectF &canvasRect,
                                   const QwtScaleMap *maps) const
{
  ...
  else
  {
    ...
    QRectF innerRect
        = canvasRect.adjusted(frameWidth, frameWidth, -frameWidth, -frameWidth);
    ...
    if (frameWidth > 0)
    {
      ...
      const int frameStyle = canvas->property("frameShadow").toInt()
          ...
          && borderRadius.value<double>() > 0.0)
      {
        ...
        QwtPainter::drawRoundedFrame(painter, canvasRect, radius, radius,
                                     canvas->palette(), frameWidth, frameStyle);
      }
      else
      {
                                          ...
                                          const QwtScaleMap maps[]) const
{
  ...
  plot->getCanvasMarginsHint(maps, canvasRect, margins[YLeft], margins[XTop],
                             margins[YRight], margins[XBottom]);
