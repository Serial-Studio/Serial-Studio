      ...
      switch (el.type)
      {
        ...
        case QPainterPath::CurveToDataElement: {
          if (clipRects.size() > 0)
          {
            ...
            r.setCoords(qwtMinF(r.left(), el.x), qwtMinF(r.top(), el.y),
                        qwtMaxF(r.right(), el.x), qwtMaxF(r.bottom(), el.y));
...
protected:
  virtual QSize sizeMetrics() const QWT_OVERRIDE { return m_size; }
...
static void qwtDrawBackground(QPainter *painter, QWidget *canvas)
{
  ...
  (void)QMetaObject::invokeMethod(canvas, "borderPath", Qt::DirectConnection,
...
static void qwtFillBackground(QPainter *painter, QWidget *canvas)
{
  ...
  else
  {
    ...
    if (borderRadius > 0.0)
    {
      ...
      rects
          += QRectF(r.bottomRight() - QPointF(borderRadius, borderRadius), sz);
...
QwtPlotAbstractCanvas::focusIndicator() const
{
...
void QwtPlotAbstractCanvas::drawBorder(QPainter *painter)
{
  ...
  if (m_data->borderRadius > 0)
  {
    ...
    if (frameWidth > 0)
    {
      ...
      QwtPainter::drawRoundedFrame(painter, frameRect, m_data->borderRadius,
                                   m_data->borderRadius, w->palette(),
                                   frameWidth, frameShape | frameShadow);
    }
  }
  else
  {
