...
public:
  ...
  inline BezierData(const QPointF &p1, const QPointF &cp1, const QPointF &cp2,
                    const QPointF &p2)
    : m_x1(p1.x())
    , m_y1(p1.y())
    , m_cx1(cp1.x())
...
private:
  inline double midValue(double v1, double v2) { return 0.5 * (v1 + v2); }
 ...
 */
QPolygonF QwtBezier::toPolygon(const QPointF &p1, const QPointF &cp1,
 ...
 */
QPointF QwtBezier::pointAt(const QPointF &p1, const QPointF &cp1,
                           const QPointF &cp2, const QPointF &p2, double t)
{
  ...
  const double x
      = ((s * p1.x() + d1 * cp1.x()) * s + d2 * cp2.x()) * s + d3 * p2.x();
  const double y
      = ((s * p1.y() + d1 * cp1.y()) * s + d2 * cp2.y()) * s + d3 * p2.y();
