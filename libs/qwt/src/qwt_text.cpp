...
const QwtTextEngine *
TextEngineDict::textEngine(const QString &text,
...
const QwtTextEngine *
TextEngineDict::textEngine(QwtText::TextFormat format) const
{
...
{
public:
  void invalidate() { textSize = QSizeF(); }
...
double QwtText::heightForWidth(double width, const QFont &defaultFont) const
{
  ...
  if (m_data->layoutAttributes & MinimumLayout)
  {
    ...
    m_data->textEngine->textMargins(font, m_data->text, left, right, top,
                                    bottom);

    h = m_data->textEngine->heightForWidth(font, m_data->renderFlags,
                                           m_data->text, width + left + right);
...
QSizeF QwtText::textSize(const QFont &defaultFont) const
{
  ...
  {
    m_layoutCache->textSize
        = m_data->textEngine->textSize(font, m_data->renderFlags, m_data->text);
...
void QwtText::draw(QPainter *painter, const QRectF &rect) const
{
  if (m_data->paintAttributes & PaintBackground)
  {
        ...
        || m_data->backgroundBrush != Qt::NoBrush)
    {
      ...
      else
      {
        ...
        painter->drawRoundedRect(rect, m_data->borderRadius,
                                 m_data->borderRadius);
      }
  ...
  m_data->textEngine->draw(painter, expandedRect, m_data->renderFlags,
                           m_data->text);
