...
void QwtPolarLayout::LayoutData::init(const QwtPolarPlot *plot,
                                      const QRectF &rect)
{
  // legend
                              ...
                              const QRectF &boundingRect, Options options)
{
  ...
  rect.adjust(m_data->margin, m_data->margin, -((int)m_data->margin),
              -(int)(m_data->margin));
  ...
  if (!(options & IgnoreLegend)
      && m_data->legendPos != QwtPolarPlot::ExternalLegend && plot->legend()
      && !plot->legend()->isEmpty())
  {
