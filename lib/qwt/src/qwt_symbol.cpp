...
static QwtGraphic qwtPathGraphic(const QPainterPath &path, const QPen &pen,
                                 const QBrush &brush)
{
  QwtGraphic graphic;
...
static inline QRectF qwtScaledBoundingRect(const QwtGraphic &graphic,
                                           const QSizeF size)
{
...
static inline void qwtDrawSvgSymbols(QPainter *painter, const QPointF *points,
                                     int numPoints, QSvgRenderer *renderer,
                                     const QwtSymbol &symbol)
{
...
static inline void qwtDrawRectSymbols(QPainter *painter, const QPointF *points,
                                      int numPoints, const QwtSymbol &symbol)
{
  const QSize size = symbol.size();
...
static inline void qwtDrawTriangleSymbols(QPainter *painter,
...
static inline void qwtDrawLineSymbols(QPainter *painter, int orientations,
                                      const QPointF *points, int numPoints,
                                      const QwtSymbol &symbol)
{
  const QSize size = symbol.size();
...
static inline void qwtDrawStar1Symbols(QPainter *painter, const QPointF *points,
                                       int numPoints, const QwtSymbol &symbol)
{
  const QSize size = symbol.size();
  painter->setPen(symbol.pen());
  ...
  if (QwtPainter::roundingAlignment(painter))
  {
    ...
    for (int i = 0; i < numPoints; i++)
    {
      ...
      QwtPainter::drawLine(painter, qRound(r.left() + d1), qRound(r.top() + d1),
                           qRound(r.right() - d1), qRound(r.bottom() - d1));
      QwtPainter::drawLine(painter, qRound(r.left() + d1),
                           qRound(r.bottom() - d1), qRound(r.right() - d1),
                           qRound(r.top() + d1));
  ...
  else
  {
    ...
    for (int i = 0; i < numPoints; i++)
    {
      ...
      QwtPainter::drawLine(painter, r.left() + d1, r.top() + d1, r.right() - d1,
                           r.bottom() - d1);
...
static inline void qwtDrawStar2Symbols(QPainter *painter, const QPointF *points,
                                       int numPoints, const QwtSymbol &symbol)
{
  QPen pen = symbol.pen();
  if (pen.width() > 1)
...
{
public:
  PrivateData(QwtSymbol::Style st, const QBrush &br, const QPen &pn,
              const QSize &sz)
    : style(st)
    , size(sz)
    , brush(br)
...
{
  m_data
      = new PrivateData(style, QBrush(Qt::gray), QPen(Qt::black, 0), QSize());
}
 ...
 */
void QwtSymbol::drawSymbols(QPainter *painter, const QPointF *points,
                            int numPoints) const
{
  if (numPoints <= 0)
    return;
...
void QwtSymbol::drawSymbol(QPainter *painter, const QRectF &rect) const
{
  ...
  else if (m_data->style == QwtSymbol::Path)
  {
    ...
    {
      m_data->path.graphic
          = qwtPathGraphic(m_data->path.path, m_data->pen, m_data->brush);
    }
  ...
  else
  {
    ...
    const double ratio
        = qMin(rect.width() / br.width(), rect.height() / br.height());
...
/*!
   ...
   \param numPoints Number of points
 */
void QwtSymbol::renderSymbols(QPainter *painter, const QPointF *points,
                              int numPoints) const
{
  switch (m_data->style)
  {
    ...
    case QwtSymbol::Cross: {
      qwtDrawLineSymbols(painter, Qt::Horizontal | Qt::Vertical, points,
                         numPoints, *this);
      break;
    }
    case QwtSymbol::XCross: {
    ...
    case QwtSymbol::Triangle:
    case QwtSymbol::UTriangle: {
      qwtDrawTriangleSymbols(painter, QwtTriangle::Up, points, numPoints,
                             *this);
      break;
    }
    case QwtSymbol::DTriangle: {
      qwtDrawTriangleSymbols(painter, QwtTriangle::Down, points, numPoints,
                             *this);
      break;
    }
    case QwtSymbol::RTriangle: {
      qwtDrawTriangleSymbols(painter, QwtTriangle::Right, points, numPoints,
                             *this);
      break;
    }
    case QwtSymbol::LTriangle: {
      qwtDrawTriangleSymbols(painter, QwtTriangle::Left, points, numPoints,
                             *this);
      break;
    }
    case QwtSymbol::HLine: {
    ...
    case QwtSymbol::Path: {
      ...
      qwtDrawGraphicSymbols(painter, points, numPoints, m_data->path.graphic,
                            *this);
      break;
    }
    case QwtSymbol::Pixmap: {
    ...
    case QwtSymbol::Graphic: {
      qwtDrawGraphicSymbols(painter, points, numPoints, m_data->graphic.graphic,
                            *this);
      break;
    }
    case QwtSymbol::SvgDocument: {
#ifndef QWT_NO_SVG
      qwtDrawSvgSymbols(painter, points, numPoints, m_data->svg.renderer,
                        *this);
#endif
      break;
    }
