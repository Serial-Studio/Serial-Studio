...
public:
  ...
  inline ScaleData &axisData(QwtAxisId axisId) { return m_scaleData[axisId]; }
...
private:
  struct ScaleData
  {
    ScaleData() { w = h = minLeft = minRight = tickOffset = 0; }
  ...
  ScaleData &axisData(QwtAxisId axisId) { return m_scaleData[axisId]; }

  inline int axesWidth(int axisPos) const { return m_scaleData[axisPos].w; }

  inline int axesHeight(int axisPos) const { return m_scaleData[axisPos].h; }
...
public:
  struct Dimensions
  {
    ...
    inline int dimAxis(QwtAxisId axisId) const { return m_dimAxes[axisId]; }

    void setDimAxis(QwtAxisId axisId, int dim) { m_dimAxes[axisId] = dim; }

    inline int dimAxes(int axisPos) const { return m_dimAxes[axisPos]; }
    ...
    inline QRectF innerRect(const QRectF &rect) const
    {
      QRectF r(rect.x() + dimAxes(QwtAxis::YLeft),
               rect.y() + dimAxes(QwtAxis::XTop), rect.width() - dimYAxes(),
  ...
  QRectF layoutLegend(QwtPlotLayout::Options, const LayoutData::LegendData &,
                      const QRectF &, const QSize &) const;

  QRectF alignLegend(const QSize &legendHint, const QRectF &canvasRect,
                     const QRectF &legendRect) const;

  void alignScales(QwtPlotLayout::Options, const LayoutData &,
                   QRectF &canvasRect,
                   QRectF scaleRect[QwtAxis::AxisPositions]) const;

  Dimensions layoutDimensions(QwtPlotLayout::Options, const LayoutData &,
                              const QRectF &) const;
...
QRectF QwtPlotLayoutPrivate::LayoutEngine::layoutLegend(
    QwtPlotLayout::Options options, const LayoutData::LegendData &legendData,
...
QRectF
QwtPlotLayoutPrivate::LayoutEngine::alignLegend(const QSize &legendHint,
...
int QwtPlotLayoutPrivate::LayoutEngine::heightForWidth(
    ...
    QwtPlotLayout::Options options, double width, int axesWidth) const
{
...
QwtPlotLayoutPrivate::LayoutEngine::Dimensions
QwtPlotLayoutPrivate::LayoutEngine::layoutDimensions(
    QwtPlotLayout::Options options, const LayoutData &layoutData,
    const QRectF &rect) const
{
  ...
  while (!done)
  {
    ...
    {
      const int d = heightForWidth(LayoutData::Title, layoutData, options,
    ...
    {
      const int d = heightForWidth(LayoutData::Footer, layoutData, options,
                                   rect.width(), dimensions.dimYAxes());
    ...
    {
      {
        ...
        if (scaleData.isVisible)
        {
          ...
          else // y axis
          {
            ...
            if (dimensions.dimAxes(XTop) > 0)
            {
              length += qMin(layoutData.tickOffset(XTop),
...
void QwtPlotLayoutPrivate::LayoutEngine::alignScales(
    QwtPlotLayout::Options options, const LayoutData &layoutData,
    QRectF &canvasRect, QRectF scaleRect[QwtAxis::AxisPositions]) const
{
  using namespace QwtAxis;
  ...
  {
    {
      ...
      if (isXAxis(axisPos))
      {
        ...
        else
        {
          ...
          {
            canvasRect.setLeft(
                qwtMaxF(canvasRect.left(), axisRect.left() - leftOffset));
          }
          else
          {
            if (leftOffset > 0)
        ...
        else
        {
          ...
          {
            canvasRect.setRight(
                qwtMinF(canvasRect.right(), axisRect.right() + rightOffset));
          }
          else
          {
            if (rightOffset > 0)
      ...
      else // y axes
      {
        ...
        if (bottomScaleRect.isValid())
        {
          ...
          {
            const double maxBottom
                = bottomScaleRect.top() + layoutData.tickOffset(XBottom);
        ...
        else
        {
          if (m_alignCanvas[XBottom] && bottomOffset < 0)
          {
            canvasRect.setBottom(
                qwtMinF(canvasRect.bottom(), axisRect.bottom() + bottomOffset));
          }
          else
          {
            if (bottomOffset > 0)
        ...
        if (topScaleRect.isValid())
        {
          ...
          {
            const double minTop
                = topScaleRect.bottom() - layoutData.tickOffset(XTop);
        ...
        else
        {
          ...
          {
            canvasRect.setTop(
                qwtMaxF(canvasRect.top(), axisRect.top() - topOffset));
          }
          else
          {
            if (topOffset > 0)
...
/*!
   ...
   \param axisPos One of QwtAxis::Position. Specifies where the position of the
   margin. -1 means margin at all borders. \sa canvasMargin()
...
{
  m_data->titleRect = m_data->footerRect = m_data->legendRect
      = m_data->canvasRect = QRectF();
 ...
 */
void QwtPlotLayout::activate(const QwtPlot *plot, const QRectF &plotRect,
                             Options options)
{
  invalidate();
  ...
  if (dimensions.dimTitle > 0)
  {
    ...
    labelRect.setRect(rect.left(), rect.top(), rect.width(),
                      dimensions.dimTitle);
  ...
  m_data->engine.alignScales(options, layoutData, m_data->canvasRect,
                             m_data->scaleRects);
