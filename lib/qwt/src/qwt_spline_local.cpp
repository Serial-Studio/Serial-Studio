...
static inline double qwtSlopeCardinal(double dx1, double dy1, double s1,
...
static inline double qwtSlopeParabolicBlending(double dx1, double dy1,
                                               double s1, double dx2,
                                               double dy2, double s2)
{
  Q_UNUSED(dy1)
  Q_UNUSED(dy2)
...
static inline double qwtSlopePChip(double dx1, double dy1, double s1,
                                   double dx2, double dy2, double s2)
{
...
public:
  ...
  inline void start(const QPointF &p0, double) { path.moveTo(p0); }

  inline void addCubic(const QPointF &p1, double m1, const QPointF &p2,
                       double m2)
  {
    const double dx3 = (p2.x() - p1.x()) / 3.0;

    path.cubicTo(p1.x() + dx3, p1.y() + m1 * dx3, p2.x() - dx3,
                 p2.y() - m2 * dx3, p2.x(), p2.y());
  }
...
public:
  ...
  inline void start(const QPointF &, double m0) { *m_m++ = m0; }
...
{
  static inline double value(double dx1, double dy1, double s1, double dx2,
                             double dy2, double s2)
  {
...
static inline double qwtSlopeP3(const QPointF &p1, const QPointF &p2,
                                const QPointF &p3)
{
...
static inline SplineStore qwtSplineL1(const QwtSplineLocal *spline,
                                      const QVector<QPointF> &points)
{
...
template<class SplineStore>
static inline SplineStore qwtSplineLocal(const QwtSplineLocal *spline,
                                         const QVector<QPointF> &points)
{
  SplineStore store;
 ...
 */
QVector<QLineF>
QwtSplineLocal::bezierControlLines(const QPolygonF &points) const
{
  if (parametrization()->type() == QwtSplineParametrization::ParameterX)
  {
    using namespace QwtSplineLocalP;
