...
static inline void qwtRenderItem(QPainter *painter, const QRect &canvasRect,
                                 QwtPlotSeriesItem *seriesItem, int from,
                                 int to)
{
 ...
 */
void QwtPlotDirectPainter::drawSeries(QwtPlotSeriesItem *seriesItem, int from,
                                      int to)
{
...
bool QwtPlotDirectPainter::eventFilter(QObject *, QEvent *event)
{
  if (event->type() == QEvent::Paint)
  {
    ...
    if (m_data->seriesItem)
    {
      ...
      if (!doCopyCache)
      {
        qwtRenderItem(&painter, canvas->contentsRect(), m_data->seriesItem,
