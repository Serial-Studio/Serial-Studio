...
{
public:
  QwtPolarGridPrivate::GridData gridData[QwtPolar::ScaleCount];
  QwtPolarGridPrivate::AxisData axisData[QwtPolar::AxesCount];
  ...
  : QwtPolarItem(QwtText("Grid"))
{
  ...
  {
    QwtPolarGridPrivate::AxisData &axis = m_data->axisData[axisId];
    switch (axisId)
    {
...
void QwtPolarGrid::setAxisPen(int axisId, const QPen &pen)
{
  ...
  QwtPolarGridPrivate::AxisData &axisData = m_data->axisData[axisId];
  if (axisData.pen != pen)
  {
    axisData.pen = pen;
    itemChanged();
...
void QwtPolarGrid::showGrid(int scaleId, bool show)
{
  ...
  QwtPolarGridPrivate::GridData &grid = m_data->gridData[scaleId];
...
QPen QwtPolarGrid::majorGridPen(int scaleId) const
{
  ...
  const QwtPolarGridPrivate::GridData &grid = m_data->gridData[scaleId];
 ...
 */
void QwtPolarGrid::draw(QPainter *painter, const QwtScaleMap &azimuthMap,
                        ...
                        double radius, const QRectF &canvasRect) const
{
  ...
  if (testDisplayFlag(ClipAxisBackground))
  {
    ...
    for (int axisId = 0; axisId < QwtPolar::AxesCount; axisId++)
    {
      const QwtPolarGridPrivate::AxisData &axis = m_data->axisData[axisId];
  ...
  const QwtPolarGridPrivate::GridData &radialGrid
  ...
  const QwtPolarGridPrivate::GridData &azimuthGrid
 ...
 */
void QwtPolarGrid::drawRays(QPainter *painter, const QRectF &canvasRect,
                            ...
                            const QList<double> &values) const
{
  for (int i = 0; i < int(values.size()); i++)
  {
    ...
    if (testDisplayFlag(SmartScaleDraw))
    {
      ...
      {
        const QwtPolarGridPrivate::AxisData &axis
            = m_data->axisData[QwtPolar::AxisRight];
      ...
      {
        const QwtPolarGridPrivate::AxisData &axis
            = m_data->axisData[QwtPolar::AxisTop];
        if (axis.isVisible && axis.scaleDraw->hasComponent(bone))
          skipLine = true;
      }
      ...
      {
        const QwtPolarGridPrivate::AxisData &axis
            = m_data->axisData[QwtPolar::AxisLeft];
        if (axis.isVisible && axis.scaleDraw->hasComponent(bone))
          skipLine = true;
      }
      ...
      {
        const QwtPolarGridPrivate::AxisData &axis
            = m_data->axisData[QwtPolar::AxisBottom];
        if (axis.isVisible && axis.scaleDraw->hasComponent(bone))
          skipLine = true;
      }
    }
 ...
 */
void QwtPolarGrid::drawCircles(QPainter *painter, const QRectF &canvasRect,
                               const QPointF &pole,
                               ...
                               const QList<double> &values) const
{
  for (int i = 0; i < int(values.size()); i++)
  {
    ...
    const QwtPolarGridPrivate::GridData &gridData
        = m_data->gridData[QwtPolar::Radius];
    ...
    {
      const QwtPolarGridPrivate::AxisData &axis
          = m_data->axisData[QwtPolar::AxisAzimuth];
      if (axis.isVisible
...
void QwtPolarGrid::updateScaleDiv(const QwtScaleDiv &azimuthScaleDiv,
                                  ...
                                  const QwtInterval &interval)
{
  QwtPolarGridPrivate::GridData &radialGrid
      = m_data->gridData[QwtPolar::Radius];
  ...
  QwtPolarGridPrivate::GridData &azimuthGrid
      = m_data->gridData[QwtPolar::Azimuth];
  ...
  for (int axisId = 0; axisId < QwtPolar::AxesCount; axisId++)
  {
    ...
    if (axis.isVisible && axis.scaleDraw)
    {
      if (axisId == QwtPolar::AxisAzimuth)
      {
        ...
        if (testDisplayFlag(SmartScaleDraw))
        {
          axis.scaleDraw->enableComponent(QwtAbstractScaleDraw::Ticks,
                                          !azimuthGrid.isVisible);
        }
      }
      else
      {
...
/*!
   ...
   \param axisId axis index ( QwtPolar::AxisLeft <= axisId <=
   QwtPolar::AxisBottom) \return specified scaleDraw for axis, or NULL if axis
   is invalid. \sa azimuthScaleDraw()
 */
...
/*!
   ...
   \param axisId axis index ( QwtPolar::AxisLeft <= axisId <=
   QwtPolar::AxisBottom) \return specified scaleDraw for axis, or NULL if axis
   is invalid. \sa setScaleDraw(), azimuthScaleDraw()
 */
...
/*!
   ...
   \param axisId axis index ( QwtPolar::AxisLeft <= axisId <=
   QwtPolar::AxisBottom) \param scaleDraw object responsible for drawing scales.
...
{
  QwtPolarGridPrivate::AxisData &axisData
      = m_data->axisData[QwtPolar::AxisAzimuth];
  if (axisData.scaleDraw != scaleDraw)
  {
    delete axisData.scaleDraw;
