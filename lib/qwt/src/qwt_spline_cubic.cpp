...
public:
  ...
  inline void reset() { m_sum = m_carry = 0.0; }

  inline double value() const { return m_sum; }
...
public:
  ...
  inline void storeFirst(double, const QPointF &, const QPointF &, double b1,
                         double)
  {
    m_cv[0] = 2.0 * b1;
  }

  inline void storeNext(int index, double, const QPointF &, const QPointF &,
                        double, double b2)
  {
    m_cv[index] = 2.0 * b2;
  }

  inline void storeLast(double, const QPointF &, const QPointF &, double,
                        double b2)
  {
    m_cv[m_curvatures.size() - 1] = 2.0 * b2;
  }

  inline void storePrevious(int index, double, const QPointF &, const QPointF &,
                            double b1, double)
  {
    m_cv[index] = 2.0 * b1;
  }

  inline void closeR() { m_cv[0] = m_cv[m_curvatures.size() - 1]; }
...
public:
  ...
  inline void storeFirst(double h, const QPointF &p1, const QPointF &p2,
                         double b1, double b2)
  {
  ...
  inline void storeNext(int index, double h, const QPointF &p1,
                        const QPointF &p2, double b1, double b2)
  {
...
#  endif
  ...
  inline void storeLast(double h, const QPointF &p1, const QPointF &p2,
                        double b1, double b2)
  {
    const double s = (p2.y() - p1.y()) / h;
  ...
  inline void storePrevious(int index, double h, const QPointF &p1,
                            const QPointF &p2, double b1, double b2)
  {
#if SLOPES_INCREMENTAL
    Q_UNUSED(p1)
...
#  endif
  ...
  inline void closeR() { m_m[0] = m_m[m_slopes.size() - 1]; }
...
public:
  ...
  inline double resolved1(double x2) const { return (r - q * x2) / p; }

  inline double resolved2(double x1) const { return (r - p * x1) / q; }
...
public:
  ...
  const T &store() const { return m_store; }
...
private:
  void substitute(const QPolygonF &points, Equation2 &eqn, Equation2 &eqX)
  {
    ...
    const Equation3 eqSplineN(QPointF(points[0].x() - hn, points[n - 2].y()),
                              points[0], points[1]);
    ...
    // an equation where only 2 coefficients ( for b[n-2], b[0] ) are left
    // unknown. Each step we have an equation that depends on b[0], b[i] and
    // b[i+1] that can also be used to substitute b[i] in b). Ding so we end up
    // with another equation depending on b[n-2], b[0] only. Finally 2 equations
    // with 2 coefficients can be solved.
  ...
  void resolveSpline2(const QPolygonF &points, double b0, double bi,
                      QVector<double> &m)
  {
    const int n = points.size();
  ...
  void resolveSpline3(const QPolygonF &points, double b0, double b1,
                      QVector<double> &m)
  {
    const int n = points.size();
  ...
  void resolveSpline4(const QPolygonF &points, double b2, double b1,
                      QVector<double> &m)
  {
    const int n = points.size();
...
static void qwtSetupEndEquations(int conditionBegin, double valueBegin,
                                 int conditionEnd, double valueEnd,
...
QVector<double> QwtSplineCubic::slopes(const QPolygonF &points) const
{
  ...
  qwtSetupEndEquations(boundaryCondition(QwtSpline::AtBeginning),
                       ...
                       boundaryValue(QwtSpline::AtEnd), points, eq);
 ...
 */
QVector<QLineF>
QwtSplineCubic::bezierControlLines(const QPolygonF &points) const
{
  // as QwtSplineCubic can calculate slopes directly we can
  // use the implementation of QwtSplineC1 without any performance loss.
...
QwtSplineCubic::polynomials(const QPolygonF &points) const
{
