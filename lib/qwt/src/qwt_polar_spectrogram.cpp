 ...
 */
void QwtPolarSpectrogram::draw(QPainter *painter, const QwtScaleMap &azimuthMap,
...
{
  renderTile(azimuthMap, radialMap, pole, tileInfo->imagePos, tileInfo->rect,
             tileInfo->image);
}
  ...
  else if (m_data->colorMap->format() == QwtColorMap::Indexed)
  {
    for (int y = y1; y <= y2; y++)
    {
      ...
      for (int x = x1; x <= x2; x++)
      {
        ...
        const uint index
            = m_data->colorMap->colorIndex(256, intensityRange, value);
