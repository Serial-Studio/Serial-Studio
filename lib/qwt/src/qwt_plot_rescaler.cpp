 ...
 */
QwtPlotRescaler::QwtPlotRescaler(QWidget *canvas, QwtAxisId referenceAxis,
 ...
 */
QwtInterval QwtPlotRescaler::expandScale(QwtAxisId axisId, const QSize &oldSize,
  ...
  switch (rescalePolicy())
  {
    ...
    case Expanding: {
      if (!oldSize.isEmpty())
      {
        ...
        expanded
            = expandInterval(oldInterval, width, expandingDirection(axisId));
      }
      break;
    }
    case Fitting: {
      ...
      if (dist > 0.0)
      {
        ...
        expanded = expandInterval(intervalHint(axisId), width,
                                  expandingDirection(axisId));
      }
      break;
    }
  }
                                            ...
                                            ExpandingDirection direction) const
{
  ...
  switch (direction)
  {
    ...
    case ExpandBoth:
    default:
      expanded.setMinValue(interval.minValue() + interval.width() / 2.0
                           - width / 2.0);
