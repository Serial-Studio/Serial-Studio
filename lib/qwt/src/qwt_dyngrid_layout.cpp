...
/*!
   \brief Returns whether this layout can make use of more space than
   sizeHint().
                                           ...
                                           uint numColumns) const
{
  ...
  for (int i = 0; i < itemCount; i++)
  {
    ...
    const QRect itemGeometry(colX[col], rowY[row], colWidth[col],
                             rowHeight[row]);
...
void QwtDynGridLayout::layoutGrid(uint numColumns, QVector<int> &rowHeight,
                                  QVector<int> &colWidth) const
{
  ...
  for (int index = 0; index < m_data->itemSizeHints.count(); index++)
  {
    ...
    rowHeight[row]
        = (col == 0) ? size.height() : qMax(rowHeight[row], size.height());
    colWidth[col]
        = (row == 0) ? size.width() : qMax(colWidth[col], size.width());
  }
}
 ...
 */
void QwtDynGridLayout::stretchGrid(const QRect &rect, uint numColumns,
                                   ...
                                   QVector<int> &colWidth) const
{
  ...
  {
    int xDelta
        = rect.width() - m.left() - m.right() - (numColumns - 1) * spacing();
  ...
  if (expandV)
  {
    ...
    int yDelta
        = rect.height() - m.top() - m.bottom() - (numRows - 1) * spacing();
    for (uint row = 0; row < numRows; row++)
