...
/*!
   ...
   \sa pen(), brush()
 */
void QwtPlotHistogram::setPen(const QColor &color, qreal width,
                              Qt::PenStyle style)
{
 ...
 */
void QwtPlotHistogram::drawSeries(QPainter *painter, const QwtScaleMap &xMap,
 ...
 */
void QwtPlotHistogram::drawOutline(QPainter *painter, const QwtScaleMap &xMap,
                                   const QwtScaleMap &yMap, int from,
                                   int to) const
{
  const bool doAlign = QwtPainter::roundingAlignment(painter);

  double v0 = (orientation() == Qt::Horizontal) ? xMap.transform(baseline())
                                                : yMap.transform(baseline());
  if (doAlign)
    v0 = qRound(v0);
 ...
 */
void QwtPlotHistogram::drawColumns(QPainter *painter, const QwtScaleMap &xMap,
                                   const QwtScaleMap &yMap, int from,
                                   int to) const
{
 ...
 */
void QwtPlotHistogram::drawLines(QPainter *painter, const QwtScaleMap &xMap,
                                 const QwtScaleMap &yMap, int from,
                                 int to) const
{
  const bool doAlign = QwtPainter::roundingAlignment(painter);
...
void QwtPlotHistogram::flushPolygon(QPainter *painter, double baseLine,
                                           ...
                                           const QwtScaleMap &yMap) const
{
  ...
  if (orientation() == Qt::Horizontal)
  {
    ...
    rect.direction
        = (x < x0) ? QwtColumnRect::RightToLeft : QwtColumnRect::LeftToRight;
  }
  else
  {
    ...
    rect.direction
        = (y < y0) ? QwtColumnRect::BottomToTop : QwtColumnRect::TopToBottom;
  }
...
/*!
   ...
   symbols) it is recommended to overload drawColumn().
 */
void QwtPlotHistogram::drawColumn(QPainter *painter, const QwtColumnRect &rect,
