                             ...
                             QwtDate::IntervalType intervalType)
{
  ...
  switch (intervalType)
  {
    ...
    case QwtDate::Minute: {
      ...
      stepSize = qwtDivideInterval(intervalSize, numSteps, limits,
                                   sizeof(limits) / sizeof(int));
                                 ...
                                 QwtDate::IntervalType intervalType)
{
  ...
  switch (intervalType)
  {
    ...
    case QwtDate::Minute: {
      ...
      {
        numSteps = qwtStepCount(stepSize, maxMinSteps, limits,
                                sizeof(limits) / sizeof(int));
      }
      else
      {
        numSteps = qwtStepCount(stepSize * 60, maxMinSteps, limits,
                                sizeof(limits) / sizeof(int));
      }
    ...
    case QwtDate::Day: {
      ...
      else
      {
        ...
        numSteps = qwtStepCount(stepSize * 24, maxMinSteps, limits,
                                sizeof(limits) / sizeof(int));
      }
    ...
    case QwtDate::Week: {
      ...
      else
      {
        ...
        {
          minStepSize = QwtScaleArithmetic::divideInterval(stepSizeInWeeks,
                                                           maxMinSteps, 10);
        }
      }
      break;
    ...
    case QwtDate::Month: {
      ...
      int numSteps = qwtStepCount(stepSize, maxMinSteps, limits,
                                  sizeof(limits) / sizeof(int));
    ...
    case QwtDate::Year: {
      ...
      {
        minStepSize
            = QwtScaleArithmetic::divideInterval(stepSize, maxMinSteps, 10);
      }
      else
      {
        // something in months
        ...
        int numSteps = qwtStepCount(12 * stepSize, maxMinSteps, limits,
                                    sizeof(limits) / sizeof(int));
...
static QList<double> qwtDstTicks(const QDateTime &dateTime, int secondsMajor,
                                 int secondsMinor)
{
  if (secondsMinor <= 0)
    QList<double>();
...
static QwtScaleDiv qwtDivideToSeconds(const QDateTime &minDate,
                                      const QDateTime &maxDate, double stepSize,
                                      int maxMinSteps,
                                      QwtDate::IntervalType intervalType)
{
  ...
  for (QDateTime dt = minDate; dt <= maxDate; dt = dt.addSecs(secondsMajor))
  {
    ...
    if (secondsMinor > 0.0)
    {
      ...
      for (int i = 1; i < numMinorSteps; i++)
      {
        ...
        {
          const bool isMedium = (numMinorSteps % 2 == 0) && (i != 1)
                                && (i == numMinorSteps / 2);
                                    ...
                                    int maxMinSteps)
{
  ...
  for (QDateTime dt = minDate; dt <= maxDate; dt = dt.addYears(stepSize))
  {
    ...
    for (int i = 1; i < numMinorSteps; i++)
    {
      ...
      const bool isMedium = (numMinorSteps > 2) && (numMinorSteps % 2 == 0)
                            && (i == numMinorSteps / 2);
                                                       ...
                                                       int maxSteps) const
{
  ...
  const int seconds
      = qwtRoundedIntervalWidth(minDate, maxDate, QwtDate::Second);
 ...
 */
void QwtDateScaleEngine::autoScale(int maxNumSteps, double &x1, double &x2,
                                            ...
                                            double stepSize) const
{
  ...
  if (intvType == QwtDate::Millisecond)
  {
    ...
    scaleDiv = QwtLinearScaleEngine::divideScale(min, max, maxMajorSteps,
  ...
  else
  {
    ...
    scaleDiv = buildScaleDiv(minDate, maxDate, maxMajorSteps, maxMinorSteps,
                             intvType);
...
QwtScaleDiv QwtDateScaleEngine::buildScaleDiv(
    const QDateTime &minDate, const QDateTime &maxDate, int maxMajorSteps,
    int maxMinorSteps, QwtDate::IntervalType intervalType) const
{
  // calculate the step size
  const double stepSize
      = qwtDivideScale(qwtIntervalWidth(minDate, maxDate, intervalType),
  ...
  {
    scaleDiv = qwtDivideToSeconds(dt0, maxDate, stepSize, maxMinorSteps,
                                  intervalType);
  }
  else
  {
                                        ...
                                        bool up) const
{
  ...
  switch (intervalType)
  {
    ...
    case QwtDate::Week: {
      const QDate date
          = QwtDate::dateOfWeek0(dt.date().year(), m_data->week0Type);
