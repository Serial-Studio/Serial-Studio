...
/*!
   ...
   <li>For angles more or equal than 360 degrees above or below min(angle1,
   angle2), scale marks will not be drawn. <li>If you need a counterclockwise
   scale, use QwtScaleDiv::setInterval()
   </ul>
 */
...
void QwtRoundScaleDraw::drawLabel(QPainter *painter, double value) const
{
  ...
  const double x
      = m_data->center.x() + (radius + sz.width() / 2.0) * std::sin(arc);
  const double y
      = m_data->center.y() - (radius + sz.height() / 2.0) * std::cos(arc);

  const QRectF r(x - sz.width() / 2, y - sz.height() / 2, sz.width(),
                 sz.height());
 ...
 */
void QwtRoundScaleDraw::drawTick(QPainter *painter, double value,
                                 double len) const
{
  if (len <= 0)
    return;
...
void QwtRoundScaleDraw::drawBackbone(QPainter *painter) const
{
  ...
  painter->drawArc(QRectF(x, y, 2 * radius, 2 * radius), -a2 * 16,
                   (a2 - a1 + 1) * 16); // counterclockwise
}
