      ...
      if (data->flags & QPaintEngine::DirtyHints)
      {
        for (int i = 0; i < 8; i++)
        {
          const QPainter::RenderHint hint
              = static_cast<QPainter::RenderHint>(1 << i);
...
public:
  ...
  PathInfo(const QRectF &pointRect, const QRectF &boundingRect,
           bool scalablePen)
  ...
  inline double scaleFactorX(const QRectF &pathRect, const QRectF &targetRect,
                             bool scalePens) const
  {
    ...
    const double w
        = 2.0 * qwtMinF(l, r) * targetRect.width() / pathRect.width();
    ...
    {
      const qreal pw
          = qwtMaxF(qAbs(m_boundingRect.left() - m_pointRect.left()),
  ...
  inline double scaleFactorY(const QRectF &pathRect, const QRectF &targetRect,
                             bool scalePens) const
  {
    ...
    const qreal h
        = 2.0 * qwtMinF(t, b) * targetRect.height() / pathRect.height();
    ...
    {
      const qreal pw
          = qwtMaxF(qAbs(m_boundingRect.top() - m_pointRect.top()),
...
void QwtGraphic::renderGraphic(QPainter *painter,
                               QTransform *initialTransform) const
{
  if (isNull())
    return;
  ...
  {
    qwtExecCommand(painter, commands[i], m_data->renderHints, transform,
                   initialTransform);
  }
 ...
 */
void QwtGraphic::render(QPainter *painter, const QPointF &pos,
 ...
 */
void QwtGraphic::drawPixmap(const QRectF &rect, const QPixmap &pixmap,
