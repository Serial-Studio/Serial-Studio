...
///
/// Every commands has got 16 bits to add subtype information (and generate
/// another unique-number this way)
enum CoalesceId
{
  ...
  = 1 << 16, ///< Selection coalescing  (we use 2 bytes for sub-types)
...
/// Though the name is 'command' these commands are NOT used on the undo stack.
/// The undostack makes use of TextChange objects!
///
/// These commands can be placed in the TextEditorCommandMap with a given name
/// By linking this command-name in the Keymap you can register keyboard
/// operations this way
