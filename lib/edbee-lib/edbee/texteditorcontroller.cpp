...
TextEditorController::TextEditorController(TextEditorWidget *widget,
                                           QObject *parent)
  : QObject(parent)
  , widgetRef_(widget)
  ...
  , borderedTextRanges_(nullptr)
{
  ...
  // create a text document (this should happen AFTER the creation of the
  // renderer)
...
void TextEditorController::notifyStateChange()
{
  if (widgetRef_)
  {
    ...
    // scrolling is only required when focused (When scrolling without focus the
    // sync-editor goes wacko :P )
    if (autoScrollToCaret_ == AutoScrollAlways
        || (autoScrollToCaret_ == AutoScrollWhenFocus && hasFocus()))
    {
...
/// sets the document and transfers the ownership of the textdocument to this
/// class
...
void TextEditorController::giveTextDocument(TextDocument *doc)
{
  if (doc != textDocument_)
  {
    setTextDocument(doc);
    textDocument_
        = doc; // this is all that's required for getting the ownership
  }
}
...
void TextEditorController::setTextDocument(TextDocument *doc)
{
  ...
  if (doc != textDocumentRef_)
  {
    ...
    if (oldDocumentRef)
    {
      ...
      disconnect(oldDocumentRef,
                 SIGNAL(textChanged(edbee::TextBufferChange, QString)), this,
                 SLOT(onTextChanged(edbee::TextBufferChange, QString)));
      disconnect(textDocumentRef_->lineDataManager(),
                 SIGNAL(lineDataChanged(int, int, int)), this,
                 SLOT(onLineDataChanged(int, int, int)));
    }
    ...
    textCaretCache_ = new TextCaretCache(
        textDocumentRef_, textRenderer_); // warning the cache needs to be
    ...
    connect(textDocumentRef_,
            SIGNAL(textChanged(edbee::TextBufferChange, QString)), this,
            SLOT(onTextChanged(edbee::TextBufferChange, QString)));
    connect(textDocumentRef_->lineDataManager(),
            SIGNAL(lineDataChanged(int, int, int)), this,
            SLOT(onLineDataChanged(int, int, int)));

    // force an repaint when the grammar is changed
    connect(textDocumentRef_, &TextDocument::languageGrammarChanged, this,
            &TextEditorController::update);

    // connect the configuration changes to the controller
    connect(textDocumentRef_->config(), &TextEditorConfig::configChanged, this,
            &TextEditorController::updateAfterConfigChange);
...
/// @param autoScroll the new autoscroll to caret setting. This can be one of
/// the following values:
///  - AutoScrollAlways => Always scroll the view so the caret is visible
//   - AutoScrollWhenFocus => Only scroll the view when the editor has got the
//   focus
//   - AutoScrollNever => Never perform automatic scrolling
void TextEditorController::setAutoScrollToCaret(
    TextEditorController::AutoScrollToCaret autoScroll)
{
...
TextEditorController::autoScrollToCaret() const
{
...
QAction *TextEditorController::createUnconnectedAction(const QString &command,
                                                       const QString &text,
                                                       const QIcon &icon,
                                                       QObject *owner)
{
...
/// @param owner the QObject owner of this action
/// @return the newly created QAction
QAction *TextEditorController::createAction(const QString &command,
                                            const QString &text,
                                            const QIcon &icon, QObject *owner)
{
  // create the action
...
/// These are textranges that are rendered with a border, but aren't truly
/// selected
...
/// gives a keymap to the editor. The ownership is transferred to this
/// controller
...
/// set a commandmap
/// the ownership is NOT transferred to this object. The old owned command-map
/// is deleted
...
/// @param searcher the new textsearcher for this document (The old one is
/// deleted)
...
TextSearcher *TextEditorController::textSearcher()
{
  if (!textSearcher_)
  {
    textSearcher_ = new TextSearcher();
  }
  return textSearcher_;
}

/// this method returns the dynamic variables object for this controller
/// The current implementation simply returns the application-wide environment
/// variables
                                         ...
                                         QString oldText)
{
  ...
  //    textSelection()->changeSpatial( change.offset(), change.length(),
  //    change.newTextLength() );
...
void TextEditorController::onLineDataChanged(int line, int length,
                                             int newLength)
{
  if (this->widgetRef_)
  {
...
/// This method is used to update the component when the configuration has been
/// changed. This is a temporary solution, perhaps we should make TextConfig
/// signal changes A lot of changes don't require an updates, but some do
...
/// This method updates the status text. This is the text as displayed in the
/// lower status bar
...
void TextEditorController::updateStatusText(const QString &extraText)
{
  ...
  else
  {
    ...
    else
    {
      ...
      QString str;
      QVector<TextScope *> scopes
          = textDocument()->scopes()->scopesAtOffset(caret);
      ...
      text.append(str);
      text.append(QObject::tr(" (%1)").arg(
          textDocument()->scopes()->lastScopedOffset()));
    }
  }
...
/// This method adds a text change on the stack that simply stores the current
/// text-selection
/// @param coalsceId the coalescing identifier for merging/coalescing undo
/// operations
...
///
/// When the name hasn't been supplied. This function assumes the command is
/// triggered by a QAction and it will retrieve the command-name from the
/// QAction data method
///
...
void TextEditorController::executeCommand(const QString &name)
{
  ...
  if (commandName.isEmpty())
  {
    ...
    // when the command name is blank  try to get the data of the QAction
    // command
    QAction *action = qobject_cast<QAction *>(sender());
    if (!action)
    {
      qlog_warn() << "executeCommand was triggered without argument and "
                     "without QAction data attribute!";
      return;
    }
...
void TextEditorController::replace(int offset, int length, const QString &text,
                                   int coalesceId, bool stickySelection)
{
...
{
  replaceRangeSet(*dynamic_cast<TextRangeSet *>(textSelection()), text,
                  coalesceId, stickySelection);
}
...
void TextEditorController::replaceSelection(const QStringList &texts,
                                            int coalesceId,
                                            bool stickySelection)
{
  replaceRangeSet(*dynamic_cast<TextRangeSet *>(textSelection()), texts,
                  coalesceId, stickySelection);
}
...
/// @param text the text to replace the selection with
/// @param coalesceId the identifier for grouping undo operations
void TextEditorController::replaceRangeSet(edbee::TextRangeSet &rangeSet,
                                           const QString &text, int coalesceId,
                                           bool stickySelection)
{
  if (readonly())
    return;
...
/// Replaces the given ranges with the given texts. Different text per range is
/// possible
...
/// This method creates a command that moves the caret to the given line/column
/// position A negative number means that we're counting from the end This
/// method assumes line 0 is the first line!
///
/// For example:
///     moveCaretTo( 2, 1 ) => Moves the caret to the 3rd line and 2nd column
///     moveCaretTo( -1, -2 ) => Moves the caret to the character before the
///     last character
///
/// The rangeIndex is used to specify which range to move.. (Defaults to -1
/// which changes to a single range)
...
/// @param keepAnchors should the anchors stay at the current position
/// (extending the selection range) The rangeIndex is used to specify which
/// range to move.. (Defaults to -1 which changes to a single range)
void TextEditorController::moveCaretToOffset(int offset, bool keepAnchors,
                                             int rangeIndex)
{
  //    SelectionCommand* command = new SelectionCommand(
  //    SelectionCommand::MoveCaretToExactOffset, offset, keepAnchors );
  SelectionCommand command(SelectionCommand::MoveCaretToExactOffset, offset,
                           keepAnchors, rangeIndex);
...
/// The rangeIndex is used to specify which range to move.. (Defaults to -1
/// which changes to a single range)
void TextEditorController::moveCaretAndAnchorToOffset(int caret, int anchor,
                                                      int rangeIndex)
{
  SelectionCommand command(SelectionCommand::MoveCaretToExactOffset, caret,
                           true, rangeIndex);
...
void TextEditorController::changeAndGiveTextSelection(
    edbee::TextRangeSet *rangeSet, int coalesceId)
{
  SelectionChange *change = new SelectionChange(this);
...
/// controller based operations are undone. When supplying false a Document
/// operation is being undone
void TextEditorController::undo(bool soft)
{
  textDocument()->textUndoStack()->undo(soft ? this : nullptr, soft);
}

/// This method performs an redo operation. By supplying soft only controller
/// based operations are redone. When supplying false a Document operation is
/// being redone
...
///                  and id > 0 means if the previous command had the same id,
///                  the command is merged
/// @param flatten when an undogroup is ended and flatten is set to true ALL
/// sub-undo-groups are merged to this group (default=false)
