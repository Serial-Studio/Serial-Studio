...
/// The texteditor works via the controller. The controller is the central
/// point/mediator which maps/controls all messages between the different editor
/// components
...
public:
  ...
  explicit TextEditorController(TextEditorWidget *widget = 0,
                                QObject *parent = 0);
  ...
  bool hasFocus();
  QAction *createUnconnectedAction(const QString &command, const QString &text,
                                   const QIcon &icon = QIcon(),
                                   QObject *owner = 0);
  QAction *createAction(const QString &command, const QString &text,
...
public slots:
  ...
  virtual void replace(int offset, int length, const QString &text,
                       int coalesceId, bool stickySelection = false);
  ...
  virtual void replaceSelection(const QStringList &texts, int coalesceId = 0,
                                bool stickySelection = false);
  virtual void replaceRangeSet(edbee::TextRangeSet &rangeSet,
                               const QString &text, int coalesceId = 0,
                               bool stickySelection = false);
  virtual void replaceRangeSet(edbee::TextRangeSet &rangeSet,
                               const QStringList &texts, int coalesceId = 0,
                               bool stickySelection = false);
  ...
  virtual void moveCaretToOffset(int offset, bool keepAnchors,
                                 int rangeIndex = -1);
  virtual void moveCaretAndAnchorToOffset(int caret, int anchor,
                                          int rangeIndex = -1);
  virtual void addCaretAt(int line, int col);
  virtual void addCaretAtOffset(int offset);
  virtual void changeAndGiveTextSelection(edbee::TextRangeSet *rangeSet,
                                          int coalesceId = 0);
...
private:
  ...
  TextCaretCache *textCaretCache_; ///< The text-caret cache. (For remembering
                                   ///< the x-position of the current carets)

  TextSearcher *textSearcher_; ///< The text-searcher

  AutoScrollToCaret autoScrollToCaret_; ///< This flags tells the editor to
                                        ///< automatically scroll to the caret
