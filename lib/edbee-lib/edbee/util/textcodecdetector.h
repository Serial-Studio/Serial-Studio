...
/// It's not possible to know which 8-bit charset is used. Except through
/// statistical analysis. we will then infer that the charset encountered is the
/// same as the default standard charset.
///
...
/// TextCodec encoding = detector.guessEncoding( QByteArray arr, QTextCode
/// fallback );
...
public:
  ...
  explicit TextCodecDetector(const QByteArray *buffer = 0,
                             TextCodec *preferedCodec = 0);
  explicit TextCodecDetector(const char *buffer, int length = 0,
                             TextCodec *preferedCodec = 0);
...
protected:
  ...
  /// If the byte has the form 11110xx, then it's the first byte of a four-bytes
  /// sequence character.
  virtual bool isFourBytesSequence(char b) { return -16 <= b && b <= -9; }

  /// If the byte has the form 11110xx, then it's the first byte of a five-bytes
  /// sequence character.
...
private:
  // const QByteArray *bufferRef_;   ///< A reference to the current buffer of
  // data
  ...
  TextCodec *fallbackCodecRef_; ///< The default codec to return. This is the
                                ///< codec to use if there's a problem detecting
                                ///< the codec or returning the prefered codec
};
