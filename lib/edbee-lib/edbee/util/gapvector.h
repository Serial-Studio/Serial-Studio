...
/// This class is used to define a gap vector. A Gapvector is split in 2 parts.
/// where the gap is moved to the insertation/changing point. So reducing the
/// movement of fields
template<typename T>
...
protected:
  ...
  {
    // qlog_info() << "** replace: " << offset << "," << length  << ":
    // gapBegin:" << gapBegin();
    //  copy the first part
    if (offset < gapBegin())
    {
    ...
    {
      // qlog_info() << "** B)
      // offset:"<<offset+",gapSize:"<<gapSize()<<",length:"<<length;
...
public:
  ...
  void append(T t) { replace(length(), 0, &t, 1); }

  /// another append method
  void append(const T *t, int length) { replace(this->length(), 0, t, length); }
  ...
  void copyRange(QChar *data, int offset, int length) const
  {
    ...
    {
      int len = qMin(gapBegin_ - offset, length);
      // qlog_info() <<  " - 1: memcpy: offset=" << offset << ", len=" << len <<
      // items_[offset];
    ...
    if (length > 0)
    {
      // qlog_info() <<  " - 2: memcpy:  offset="<<offset << "gapSize=" <<
      // gapSize()<< ", length=" << length << items_[offset + gapSize()];
  ...
  /// WARNING, this method MOVES the gap! Which means this method should NOT be
  ...
  void moveGapTo(int offset)
  {
    ...
    if (offset != gapBegin_)
    {
      // qlog_info() << "BEGIN moveGapTo: offset=" << offset << "/ gapBegin_";
      // // << gapBegin_ << getUnitTestString();
      int gapSize = this->gapSize();

      // move the the data right after the gap
      if (offset < gapBegin_)
      {
        // qlog_info() << "- move"  << offset << "," << gapBegin_ <<
        // "(charcount:" << (gapBegin_ - offset)<<")";
        memmove(items_ + offset + gapSize, items_ + offset,
                sizeof(T)
                    * (gapBegin_ - offset)); // memmove( target, source, size )
      }
      else
      {
        // qlog_info() << "- move2: gapBegin_" << gapBegin_ << ", gapEnd_" <<
        // gapEnd_ << ", capacity_" << capacity_ << ", gapSize" << gapSize <<
        // "(charcount:"<<(offset - gapBegin_)<<")";
        memmove(items_ + gapBegin_, items_ + gapEnd_,
                sizeof(T)
                    * (offset - gapBegin_)); // memmove( target, source, size )
      }
      gapBegin_ = offset;
  ...
  void resize(int newSize)
  {
    ...
    // qlog_info() << "BEGIN resize: capacity =" << capacity_<< " => " <<
    // newSize;
...
public:
  ...
  void replaceString(int offset, int length, const QString &data)
  {
    ...
    //        qlog_info() << "replace(" << offset << length << data << ") : " <<
...
/// A special GapVector class that isn't a gapvector. It forwards it's request
/// to a normal vector class (for debugging purposes) that isn't a gapv This
/// class is only used for debugging issuess with the gapvector
template<typename T>
class EDBEE_EXPORT NoGapVector
{
public:
  NoGapVector(int capacity = 16) { Q_UNUSED(capacity); }
...
public:
  ...
  /// convenient append method
  void append(T t) { items_.append(t); }
  ...
  /// This method returns the item at the given index
  T at(int offset) const { return items_.at(offset); }

  /// This method sets an item at the given index
  void set(int offset, const T &value) { items_.replace(offset, value); }

  /// This method return an index
  T &operator[](int offset) { return items_[offset]; }
  ...
  //qlog_info() << "copyRange" << offset << length;
          ...
          if( offset < gapBegin() ) {
              int len = qMin( gapBegin_-offset, length );
  //qlog_info() <<  " - 1: memcpy: offset=" << offset << ", len=" << len <<
  items_[offset]; memcpy( data, items_ + offset, sizeof(T)*len ); data      +=
  len;   // increase the pointer offset    += len; length    -= len;
          }

          if( length > 0 ) {
  //qlog_info() <<  " - 2: memcpy:  offset="<<offset << "gapSize=" <<
  gapSize()<< ", length=" << length << items_[offset + gapSize()]; memcpy( data,
  items_ + offset + gapSize(), sizeof(T)*length );
          }
      }
  */
  ...
  */
  ...
  /*
      void moveGapTo( int offset ) {
          ...
          if( offset != gapBegin_ ) {
  //qlog_info() << "BEGIN moveGapTo: offset=" << offset << "/ gapBegin_"; // <<
  gapBegin_ << getUnitTestString(); int gapSize = this->gapSize();

              // move the the data right after the gap
              if (offset < gapBegin_ ) {
  //qlog_info() << "- move"  << offset << "," << gapBegin_ << "(charcount:" <<
  (gapBegin_ - offset)<<")"; memmove( items_ + offset + gapSize, items_ +
  offset, sizeof(T) * (gapBegin_ - offset));   // memmove( target, source, size
  )

              } else {
  //qlog_info() << "- move2: gapBegin_" << gapBegin_ << ", gapEnd_" << gapEnd_
  << ", capacity_" << capacity_ << ", gapSize" << gapSize <<
  "(charcount:"<<(offset - gapBegin_)<<")"; memmove( items_ + gapBegin_, items_
  + gapEnd_, sizeof(T) * (offset - gapBegin_ ));  // memmove( target, source,
  size )
              }
              gapBegin_ = offset;
              gapEnd_   = gapBegin_ + gapSize; //qMin( gapBegin_ + gapSize,
  capacity_ );
          }
          Q_ASSERT( gapBegin_ <= gapEnd_ );
