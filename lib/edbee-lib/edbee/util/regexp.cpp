...
// This is required for windows, to prevent linkage errors (somehow the sources
// of oniguruma assumes we're linking with a dll)
...
private:
  ...
  /// fills the error with the given code. It retrives the onig_error_code if
  /// requried
...
public:
  ...
  OnigRegExpEngine(const QString &pattern, bool caseSensitive,
    ...
    , lineRef_(0)
  {
    ...
    if (syntax == RegExp::SyntaxFixedString)
    {
    ...
    if (!caseSensitive)
    {
      onigOptions = onigOptions | ONIG_OPTION_IGNORECASE;
    }

    int result
        = onig_new(&reg_, (OnigUChar *)patternChars,
                   (OnigUChar *)(patternChars + pattern.length()), onigOptions,
                   ONIG_ENCODING_UTF16_LE, onigSyntax, &einfo_);
  ...
  int indexIn(const QChar *charPtr, int offset, int length, bool reverse)
  {
    ...
    int result = onig_search(reg_, stringStart, stringEnd, stringOffset,
                             stringRange, region_, ONIG_OPTION_NONE);
    if (result >= 0)
    {
      Q_ASSERT(result % 2 == 0);
      return result >> 1;
  ...
  {
    line_ = str;
    int length = line_.length(); // very scary, calling line_.length()
                                 // invalidates the line_.data() pointer :S
    lineRef_ = line_.data();
  ...
  virtual int pos(int nth) const
  {
    ...
    if (nth < region_->num_regs)
    {
      ...
      if (result % 2 != 0)
      {
        ...
        {
          qlog_info() << QStringLiteral(" - %1: (%2,%3)")
                             .arg(i)
                             .arg(region_->beg[i])
                             .arg(region_->end[i]);
        }
  ...
  virtual int len(int nth) const
  {
    ...
    if (nth < region_->num_regs)
    {
      int result
          = region_->end[nth]
            - region_->beg[nth]; // end is the first character AFTER the match
      Q_ASSERT(result % 2 == 0);
      return result >> 1;
    }
    return -1;
...
public:
  ...
  QtRegExpEngine(const QString &pattern, bool caseSensitive,
                 RegExp::Syntax syntax)
    : reg_(0)
  {
    ...
    if (syntax == RegExp::SyntaxFixedString)
    {
      regExpSyntax = QRegExp::FixedString;
    }

    reg_ = new QRegExp(pattern,
                       caseSensitive ? Qt::CaseSensitive : Qt::CaseInsensitive,
                       regExpSyntax);
  }

  /// destructs the QtRegExpEngine
  virtual ~QtRegExpEngine() { delete reg_; }
...
/// @param syntax the syntax of th given regular expression
/// (SyntaxDefault(default) or SyntaxFixedString)
/// @param engine the engine to use (EngineOniguruma(default) or EngineQRegExp)
RegExp::RegExp(const QString &pattern, bool caseSensitive, Syntax syntax,
               Engine engine)
  : d_(0)
{
  switch (engine)
  {
    ...
    default:
      Q_ASSERT(false);
      qlog_warn() << "Invalid engine supplied to RegExp. Falling back to "
                     "EngineOniguruma";
      d_ = new OnigRegExpEngine(pattern, caseSensitive, syntax);
      break;
  }
...
/// we currently always use QRegExp::escape.. For the future we added an engine
/// parameter which is currently ignored
...
/// offset is -1, the search starts at the last character; if -2, at the next to
/// last character; etc. Returns the position of the first match, or -1 if there
/// was no match. The caretMode parameter can be used to instruct whether ^
/// should match at index 0 or at offset. You might prefer to use
/// QString::indexOf(), QString::contains(), or even QStringList::filter(). To
/// replace matches use QString::replace().
...
/// Returns the position of the nth captured text in the searched string. If nth
/// is 0 (the default), pos() returns the position of the whole match. For
/// zero-length matches, pos() always returns -1. (For example, if cap(4) would
/// return an empty string, pos(4) returns -1.) This is a feature of the
/// implementation.
