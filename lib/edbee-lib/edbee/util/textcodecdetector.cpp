...
TextCodecDetector::TextCodecDetector(const QByteArray *buffer,
                                     TextCodec *preferedCodec)
...
TextCodecDetector::TextCodecDetector(const char *buffer, int length,
                                     TextCodec *preferedCodec)
  : bufferRef_(buffer)
...
void TextCodecDetector::setFallbackCodec(TextCodec *codec)
{
  ...
  {
    /// prefer System
    fallbackCodecRef_ = Edbee::instance()->codecManager()->codecForName(
        "ISO-8859-1"); // QTextCodec::codecForLocale();    // is identical to:
...
///  readable text file.
///
/// If there is no BOM, this method tries to discern whether the file is UTF-8
/// or not. If it is not UTF-8, we assume the encoding is the default system
/// encoding (of course, it might be any 8-bit charset, but usually, an 8-bit
/// charset is the default one)
///
/// It is possible to discern UTF-8 thanks to the pattern of characters with a
/// multi-byte sequence
///
/// @code
...
TextCodec *TextCodecDetector::detectCodec()
{
  ...
  if (hasUTF8Bom(bufferRef_, bufferLength_))
    return codecManager()->codecForName("UTF-8 with BOM");
  if (hasUTF16LEBom(bufferRef_, bufferLength_))
    return codecManager()->codecForName("UTF-16LE with BOM");
  if (hasUTF16BEBom(bufferRef_, bufferLength_))
    return codecManager()->codecForName("UTF-16BE with BOM");
  if (hasUTF32LEBom(bufferRef_, bufferLength_))
    return codecManager()->codecForName("UTF-32LE with BOM");
  if (hasUTF32BEBom(bufferRef_, bufferLength_))
    return codecManager()->codecForName("UTF-32BE with BOM");

  // if a byte has its most significant bit set, the file is in UTF-8 or in the
  // default encoding otherwise, the file is in US-ASCII
  bool highOrderBit = false;

  // if the file is in UTF-8, high order bytes must have a certain value, in
  // order to be valid if it's not the case, we can assume the encoding is the
  ...
  // (it might have been UTF-7, but this encoding is usually internally used
  // only by mail systems)
  if (!highOrderBit)
  {
  ...
  // finally, if it's not UTF-8 nor US-ASCII, let's assume the encoding is the
  // default encoding
