...
TestResult::TestResult(TestCase *testCase, const QString &methodName,
                       const QString &description, const char *file,
                       int lineNumber)
...
void TestResult::setCompareResult(bool result, const QString &actualValue,
...
void TestCase::testTrueImpl(bool condition, const char *statement,
                            const QString &description, const char *file,
                            int line)
{
  TestResult *testResult
      = new TestResult(engine()->currentTest(), engine()->currentMethodName(),
...
/// @param file the source file
/// @param line the line number
void TestCase::testEqualImpl(bool result, const QString &actual,
                             ...
                             int line)
{
  TestResult *testResult
      = new TestResult(engine()->currentTest(), engine()->currentMethodName(),
                       description, file, line);
  testResult->setCompareResult(result, actual, expected, actualStatement,
                               expectedStatement);
  engine()->giveTestResult(testResult);
}
...
/// @param file the source file
/// @param line the line number
void TestCase::testSkipImpl(const QString &description, const char *file,
                            int line)
{
  TestResult *testResult
      = new TestResult(engine()->currentTest(), engine()->currentMethodName(),
  ...
  foreach (TestResult *result, engine->testResultList())
  {
    switch (result->status())
    {
      ...
      case TestResult::Skipped:
        ...
        break;
      default:
        Q_ASSERT_X(false, "TestResult Status", "Unkown status returned!");
    }
  }

  qDebug() << " ** completed: " << passedCount << " passed,  " << failedCount
           << " failures, " << skippedCount << " skipped";
  if (failedCount > 0)
  {
...
void OutputHandler::endTestMethod(TestEngine *engine)
{
  ...
  // write the result
  qDebug() << ""
           << (const char *)QStringLiteral("%1%2%3")
                  .arg(buffer_)
                  .arg(failBuffer_)
                  .arg(outBuffer_)
                  .toLatin1();
...
void OutputHandler::testResultAdded(TestEngine *engine, TestResult *testResult)
{
  ...
  switch (testResult->status())
  {
    ...
    case TestResult::Failed:
      ...
      {
        failBuffer_.append("\n");
        failBuffer_.append("    [FAILURE] ")
            .append(testResult->actualStatement())
            .append(" == ")
            .append(testResult->expectedStatement())
            .append("\n");
        if (!testResult->description().isEmpty())
          failBuffer_.append("      ")
              .append(testResult->description())
              .append("\n");
        failBuffer_.append("      expected: [")
            .append(testResult->expectedValue())
            .append("]\n");
        failBuffer_.append("      actual  : [")
            .append(testResult->actualValue())
            .append("]\n");
        failBuffer_.append("      ")
            .append(testResult->fileName())
            .append(":")
            .append(QStringLiteral("%1").arg(
                testResult->lineNumber())); //.append("\n");
      }
      else
      {
        failBuffer_.append("\n");
        failBuffer_.append("    [FAILURE] ")
            .append(testResult->statement())
            .append("\n");
        if (!testResult->description().isEmpty())
          failBuffer_.append("      ")
              .append(testResult->description())
...
{
  if (testRefList_.contains(object))
    return true;
...
int TestEngine::run(TestCase *test)
{
  ...
  if (initTestCaseIndex >= 0)
    metaObject->invokeMethod(test, "initTestCase");

  // this method invokes all private slots
  for (int methodIdx = 0, methodCount = metaObject->methodCount();
       methodIdx < methodCount; ++methodIdx)
  {
        ...
        && method.methodType() == QMetaMethod::Slot)
    {
      ...
      if (currentMethodName_.startsWith('_'))
        continue; // skip underscore names

      if (currentMethodName_.compare("init()") == 0
          || currentMethodName_.compare("clean()") == 0
          || currentMethodName_.compare("initTestCase()") == 0
          || currentMethodName_.compare("cleanupTestCase()") == 0)
        continue;
      ...
      if (initTestIndex >= 0)
        metaObject->invokeMethod(test, "init");
      method.invoke(test);
      if (cleanTestIndex >= 0)
        metaObject->invokeMethod(test, "clean");
  ...
  if (cleanTestCaseIndex >= 0)
    metaObject->invokeMethod(test, "cleanupTestCase");
...
/// should be called before the first run when manually running single unit
/// tests When using runAll you should not call this method
