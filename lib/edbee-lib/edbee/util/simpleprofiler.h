...
#  define PROF_END SimpleProfiler::instance()->end();
...
/// A simple profiler class that can be used to profile certain parts of the
/// code. I've introduced this class as a poor-mans profiler. Because currently
/// valgrand and other profilers don't seem to run smoothly on my mac. They
/// crash ..
...
public:
  ...
  {
  public:
    ProfilerItem(const char *filename, int line, const char *function,
                 const char *name);
  ...
  protected:
    ...
    qint64
        childDuration_; ///< Duration of child-items (items called by this item)
  };
  ...
  void begin(const char *file, int line, const char *function,
             const char *name);
  void end();
