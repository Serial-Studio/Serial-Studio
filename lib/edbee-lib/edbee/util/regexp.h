...
/// We need this Regular Expression library to be able to support tmLanguages
/// fully I tried to make this class as close as possible to the QRegExp library
...
public:
  enum Engine
  {
    EngineOniguruma = 1,
    EngineQRegExp = 2
    //        QRegExp::RegExp	0	A rich Perl-like pattern matching
    //        syntax. This is the default. QRegExp::RegExp2	3	Like
    //        RegExp, but with greedy quantifiers. (Introduced in Qt 4.2.)
    //        QRegExp::Wildcard	1	This provides a simple pattern matching
    //        syntax similar to that used by shells (command interpreters) for
    //        "file globbing". See QRegExp wildcard matching.
    //        QRegExp::WildcardUnix	4	This is similar to Wildcard but
    //        with the behavior of a Unix shell. The wildcard characters can be
    //        escaped with the character "\". QRegExp::FixedString	2
    //        The pattern is a fixed string. This is equivalent to using the
    //        RegExp pattern on a string in which all metacharacters are escaped
    //        using escape(). QRegExp::W3CXmlSchema11	5	The pattern is a
    //        regular expression as defined by the W3C XML Schema 1.1
    //        specification.
  };
  ...
  RegExp(const QString &pattern, bool caseSensitive = true,
         Syntax syntax = SyntaxDefault, Engine engine = EngineOniguruma);
  virtual ~RegExp();
