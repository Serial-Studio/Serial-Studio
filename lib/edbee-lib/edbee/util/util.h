...
/// The purpose of this class is to put 'global' function that don't quite fit
/// on other places
///
...
public:
  ...
  /// @param exclusive if it is exclusive then an overlap will not be included.
  /// In other words:
  ///			- Inclusive:  end < begin
  ///			- Exclusive:  end <= begin
  ...
  template<typename T>
  bool intersection(T begin1, T end1, T begin2, T end2, bool exclusive = false,
                    T *resultBegin = 0, T *resultEnd = 0)
  {
    if (exclusive)
    {
      if (end1 <= begin2)
    ...
    if (resultBegin)
    {
      *resultBegin = qMax(begin1, begin2);
    }
    if (resultEnd)
    {
      *resultEnd = qMin(end1, end2);
    }
    return true;
  }
};
