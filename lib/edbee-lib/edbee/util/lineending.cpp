...
LineEnding::LineEnding(LineEnding::Type type, const char *chars,
                       const char *escaped, const char *name)
  : type_(type)
  , charsRef_(chars)
...
/// @param unknown the type to return when the ending is not nown. (defaults to
/// 0)
  ...
  for (int i = 0, cnt = str.length(); i < cnt; i++)
  {
    ...
    if (c1 == '\r')
    {
      if (c2 == '\n')
      {
        ++winCount;
        ++i;
        if (winCount >= endLoopWhenCountReaches)
          break;
      }
      else
      {
        ++macClassicCount;
        if (macClassicCount >= endLoopWhenCountReaches)
          break;
      }
    }
    else if (c1 == '\n')
    {
      ++unixCount;
      if (unixCount >= endLoopWhenCountReaches)
        break;
    }
  }
  if (macClassicCount > unixCount && macClassicCount > winCount)
    return get(LineEnding::MacClassicType);
  if (winCount > unixCount)
    return get(LineEnding::WindowsType);
  if (unixCount > 0)
    return get(LineEnding::UnixType);
  return unkown;
}
