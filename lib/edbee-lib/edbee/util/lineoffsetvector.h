...
/// This class implements the vector for storing the line numbers at certain
/// offsets/ The class allows the 'gap' position to contain a delta offset.
/// Which means that
///
/// all offsets after the gap are increased with the offsetDelta when searched.
/// Inserting/deleting text this way usually only results in the changine of the
/// offset delta. Which means speeeed
///
/// The line offset pointed at by each index is the first character in the given
/// line.
...
public:
  ...
  //    struct LineChange {
  //        int line;                   ///< the first line that's going to be
  //        replaced int lineCount;              ///< the number of lines that
  //        are replaced int newLineCount;           ///< the number of lines
  //        added/removed int offsetDelta;            ///< the change in offset
