...
{
  edbee::DebugAllocationList *allocList
      = edbee::DebugAllocationList::instance();
  ...
  if (allocList->isRunning())
  {
    ...
    if (info != nullptr)
    {
      ...
      printf("existing info : %p(%u) %s:%d\n", info->pointer,
             static_cast<unsigned int>(info->size), info->file, info->line);
      ::free(p);
      return nullptr;
    }
    info = allocList->add(p, size, const_cast<char *>(file),
...
void debug_free(void *p, const char *file, const int line)
{
  ...
  if (allocList->isRunning())
  {
    if (!allocList->del(p))
    {
      // TODO: This doesn't work very good. It sees to many free that are NOT
      // allocated by us
