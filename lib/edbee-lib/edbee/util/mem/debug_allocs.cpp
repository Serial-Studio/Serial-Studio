...
DebugAllocationList::~DebugAllocationList()
{
  ...
  // delete mutex_;   // < do not delete keep memory leak :S. When deleting this
  // mutex app shutdown crashes
  mutex_ = 0;
  clear();
}
...
void DebugAllocationList::clear()
{
  ...
  for (std::map<void *, DebugAllocation *>::iterator iter
       = allocationList_.begin();
       iter != allocationList_.end(); ++iter)
  {
       ...
       iter != allocationList_.end(); ++iter)
  {
    ...
    if (info && info->pointer)
    {
      printf("Memory leak %p(%u) %s:%d\n", info->pointer,
             (unsigned int)info->size, info->file, info->line);
      res++;
    }
  }
...
DebugAllocation *DebugAllocationList::add(void *p, size_t size, char *file,
                                          int line)
{
  DebugAllocation *info
      = (DebugAllocation *)std::malloc(sizeof(DebugAllocation));
  info->clear();
  info->pointer = p;
  info->size = size;
  info->file = file;
