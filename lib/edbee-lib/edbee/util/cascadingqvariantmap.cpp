...
/// Tough this doesnt' feel quite correct (it uses references to parent) it
/// makes managing the map a bit easier.
...
/// Returns the value of the given key. If the map at this level doesn't have
/// this key, it cascades to a higher-level. If it doesn't find the key if will
/// fallback to the defValue supplied
/// @param key the name of the item to retrieve
/// @param defValue the default value when the item isn't found
const QVariant CascadingQVariantMap::value(const QString &key,
...
const QString CascadingQVariantMap::stringValue(const QString &key,
...
double CascadingQVariantMap::doubleValue(const QString &key,
