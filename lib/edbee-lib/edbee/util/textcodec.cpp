...
TextCodecManager::TextCodecManager()
{
  ...
  encList << "UTF-8" << "UTF-16" << "UTF-16BE" << "UTF-16LE" << "UTF-32"
          << "UTF-32BE" << "UTF-32LE";
  foreach (QByteArray enc, encList)
  {
    QTextCodec *codec = QTextCodec::codecForName(enc);
    giveTextCodec(
        new TextCodec(QString(codec->name()), codec, QTextCodec::IgnoreHeader));
    giveTextCodec(
        new TextCodec(QStringLiteral("%1 with BOM").arg(QString(codec->name())),
                      codec, QTextCodec::DefaultConversion));
  }
  ...
  foreach (QByteArray name, names)
  {
    ...
    {
      TextCodec *textCodec
          = new TextCodec(name, codec, QTextCodec::DefaultConversion);
...
/// Registers the given codec. The ownership of this codec is transfered to the
/// codec manager
...
/// @param the QTextCodec conversion flags (is used for creating codecs with and
/// without BOM)
TextCodec::TextCodec(const QString &name, const QTextCodec *codec,
