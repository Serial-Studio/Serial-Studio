...
#define DECLARE_TEST(className)                                                \
  static edbee::test::Test<className> TEST_MAKE_UNIQUE(t)(#className)
#define DECLARE_NAMED_TEST(name, className)                                    \
  static edbee::test::Test<className> TEST_MAKE_UNIQUE(name)(#className)
...
public:
  ...
  explicit TestResult(TestCase *testCase, const QString &methodName,
                      const QString &description, const char *file,
                      int lineNumber);
  virtual ~TestResult() {}

  virtual void setBooleanResult(bool result, const char *statement);
  // virtual void setCompareResult( const QString& actualValue, const QString&
  // expectedValue, const char* actualStatement, const char* expectedStatement
  // );

  virtual void setCompareResult(bool result, const QString &actualValue,
                                const QString &expectedValue,
                                const char *actualStatement,
                                const char *expectedStatement);

  //    template <class T>
  //    void setCompareResult( const T& actualValue, const T& expectedValue,
  //    const char* actualStatement, const char* expectedStatement )
  //    {
  //        setCompareResult( actualValue == expectedValue,
  //        toQString(actualValue), toQString(expectedValue), actualStatement,
  //        expectedStatement );
  //    }
...
#define testTrue_2(statement, message)                                         \
  testTrueImpl((statement) ? true : false, #statement, (message), __FILE__,    \
               __LINE__)
...
#define testEqual_1(actual, expected)                                          \
  ...
  {                                                                            \
    ...
    testEqualImpl((actualStr) == (expectedStr), actualStr, expectedStr,        \
...
#define testEqual_2(actual, expected, message)                                 \
  do                                                                           \
    QString actualStr = edbee::test::toQString(actual);                        \
  QString expectedStr = edbee::test::toQString(expected);                      \
  testEqualImpl((actualStr) == (expectedStr), actualStr, expectedStr, #actual, \
                #expected, (message), __FILE__, __LINE__);                     \
  }                                                                            \
  while (false)
...
/// I really really hate the QTestLib output on my Mac. It's a very ugly
/// xwindows console result.
///
/// This extemely simple testing support class makes it possible for met to
/// output the test results to a WebKit window. Where I can added clickable divs
/// etc. to show test result details
///
/// The basic conventions for these tests are the sames as those of QTest. The
/// main difference is the usage of exta output/compare info output messsages
...
public:
  ...
  virtual void testTrueImpl(bool condition, const char *statement,
                            const QString &description, const char *file,
                            int line);

  virtual void
  testEqualImpl(bool result, const QString &actual, const QString &expected,
                const char *actualStatement, const char *expectedStatement,
                const QString &description, const char *file, int line);

  virtual void testSkipImpl(const QString &description, const char *file,
                            int line);
...
private:
  ...
  //    const char* currentMethodNameRef_;  ///< A reference to the current
  //    method
