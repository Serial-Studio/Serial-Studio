...
void LineOffsetVector::applyChange(TextBufferChange change)
{
  ...
  //     if( i ) { offsets.append(","); }
  //     offsets.append(
  //     QStringLiteral("%1").arg(change.newLineOffsets().at(i)));
  //     //qlog_info() << "-- " << i << ":" << change.newLineOffsets().at(i);
  // }

  // qlog_info() << "- before: " << toUnitTestString() ;
  // qlog_info() << "- offsetList_.replace(" << change.line() << "," <<
  // change.lineCount() << ": " << offsets << ")";
  offsetList_.replace(line, change.lineCount(),
                      change.newLineOffsets().constData(),
                      change.newLineCount());
...
void LineOffsetVector::textReplaced(int offset, int length, const QChar *newData
, int newDataLength)
{
    LineChange change;
...
int LineOffsetVector::findLineFromOffset(int offset)
{
  ...
  {
    line = searchOffsetIgnoringOffsetDelta(offset - offsetDelta_,
                                           offsetDeltaIndex_, offsetListLength);
  }
  return line;
}

/// This method appends an offset to the end of the list
/// It simply applies the current offsetDelta because it will always be AFTER
/// the current offsetDelta
...
/// a binary search. Warning this method uses RAW values from the offsetList it
/// does NOT take in account the offsetDelta
int LineOffsetVector::searchOffsetIgnoringOffsetDelta(int offset, int org_start,
                                                      int org_end)
{
...
void LineOffsetVector::moveDeltaToIndex(int index)
{
  if (offsetDelta_)
  {
    ...
    else if (index < offsetDeltaIndex_)
    {
      ...
      // (1) Make the delta 0 or (2) apply the 'negative index' to the items on
      // the left
