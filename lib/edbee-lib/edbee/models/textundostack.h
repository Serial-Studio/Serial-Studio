...
/// This is the undo stack for the texteditor. This stack is SHARED by all views
/// of the document The stack also stores view-specific commands of all views
///
/// Every view has got it's own pointer on the stack. This pointer points at the
/// index AFTER the last item on the stack that's used for this view.
///
/// When a view-performs a view-specific undo (soft-undo) it's own pointer is
/// stepped back. Only view-specific commands are undone When a view performs a
/// document undo. ALL view-pointers a are undone to the point of the document
/// undo.
///
/// ~~~~
///
...
public:
  /// This enumeration is signaled to the listeners to notify what happend when
  /// ending an undo group
  enum EndUndoGroupAction
  {
    ActionUngrouped,   ///< The group has been ungrouped, the single change has
                       ///< been added to the stack
    ActionFullDiscard, ///< The group contained nothing usefull, nothing is
                       ///< added to the stack
...
signals:
  ...
  /// This signal is fired when the group is ended. Warning, when the group is
  /// merged the group pointer will be 0!!
  void undoGroupEnded(int coalesceId, bool merged, int action);
  void changeAdded(edbee::Change *change);
  //    void changeMerged( edbee::TextChange* oldChange, edbee::TextChange*
  //    change );
...
private:
  ...
  QMap<TextEditorController *, int>
      controllerIndexMap_; ///< The current controller pointers (View specific)
  int persistedIndex_; ///< The current persisted index. A persisted-index of
                       ///< -1, means it never is persisted

  QStack<ChangeGroup *> undoGroupStack_; ///< A stack of all undo items
  QStack<int> lastCoalesceIdStack_;      ///< The last coalesceId

  //    int undoGroupLevel_;                                 ///< The undo group
  //    level TextChangeGroup* undoGroup_;                         ///< The
  //    current undo group
