...
/// @param selector (default=0) the selector to use. This class takes ownership
/// of the selector!!
ScopedDynamicVariable::ScopedDynamicVariable(const QVariant &value,
                                             TextScopeSelector *selector)
...
void DynamicVariables::setAndGiveScopedSelector(const QString &name,
                                                const QVariant &value,
                                                const QString &selector)
{
  /// Todo, perhaps we should detect identical scope selectors and replace the
  /// original
  variableNames_.insert(name);
  scopedVariableMap_.insert(
      name, new ScopedDynamicVariable(value, new TextScopeSelector(selector)));
}
...
/// @return the number of variable names. (there can be more values as
/// variables)
...
DynamicVariable *DynamicVariables::find(const QString &name,
                                        TextScopeList *scopelist)
{
  ...
  if (scopelist)
  {
    foreach (ScopedDynamicVariable *var, scopedVariableMap_.values(name))
    {
      ...
      // qlog_info() << "-  " << var->value() << ": score: " << score << " > "
      // << resultScore;
