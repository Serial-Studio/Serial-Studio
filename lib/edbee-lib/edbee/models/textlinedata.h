...
class EDBEE_EXPORT TextLineData{
  public : TextLineData(){} virtual ~TextLineData(){}
  //    bool undoable() = 0;
};
...
public:
  ...
  void realloc(TextLineDataManager *manager, int oldFieldPerLine,
...
public:
  ...
  // internal functions
  Change *createLinesReplacedChange(int lineStart, int lineCount,
                                    int newLineCount);
...
signals:
  ...
  void lineDataChanged(
      int line, int length,
      int newLength); ///< This signal is emitted if line-data is changed
