...
/// This method can be used to change the number of reserved fields by the
/// document Increasing the amount will result in a realoc Decreasting the
/// fieldcount  reults in the lost of the 'old' fields At least the
/// 'PredefinedFieldCount' amont of fields are required This method EMPTIES the
/// undo-stack. So after this call all undo history is gone!
...
/// This method should return true if the current change is the cause of an undo
/// operation
...
void TextDocument::replaceRangeSet(TextRangeSet &rangeSet,
                                   const QString &textIn, bool stickySelection)
{
                                   ...
                                   bool stickySelection)
{
  ...
  while (idx < (oldRangeCount = rangeSet.rangeCount()))
  {
    ...
    // qlog_info() << idx << ">> " << text << " : delta="<<delta<<", " <<
    // range.toString();
    ...
    TextChangeWithCaret *change
        = new TextChangeWithCaret(range.min(), range.length(), text, -1);
    Change *effectiveChange = executeAndGiveChange(change, false);
    TextChangeWithCaret *effectiveChangeWithCaret
        = dynamic_cast<TextChangeWithCaret *>(effectiveChange);

    // when a new caret position is supplied (can only happen via a
    // TextDocumentFilter) access it and change the caret to the given position
    int caret = 0;
    if (effectiveChangeWithCaret && effectiveChangeWithCaret->caret() >= 0)
    {
      caret = effectiveChangeWithCaret->caret();
      // Default caret location is change-independent: old location + length new
      // text
    }
    else
    {
    ...
    {
      qlog_info() << "TEST TO SEE IF THIS REALLY HAPPENS!! I think it cannot "
...
void TextDocument::giveSelection(TextEditorController *controller,
...
/// call this method to execute a change. The change is first passed to the
/// filter so the documentFilter can handle the processing of the change When
/// not filter is active the 'execute' method is called on the change
///
/// WARNING, you should never Access the change pointer given to this method
/// It's possible the change gets deleted
///
/// This method  should return the effective text-change (or 0 if no text-change
/// has been stored)
Change *TextDocument::executeAndGiveChange(Change *change, int coalesceId)
{
  ...
  else
  {
    ...
    beginUndoGroup(); // automatically group changes together (when changes
                      // happen on emission)
    change->execute(this);
...
/// @param coalesceId (default 0) the coalesceId to use. Whe using the same
/// number changes could be merged to one change. CoalesceId of 0 means no
/// merging
...
/// number changes could be merged to one change. CoalesceId of 0 means no
/// merging
void TextDocument::replace(int offset, int length, const QString &text,
                           int coalesceId)
{
...
/// to the textdocument-buffer. No undo-data is collected and no events are
/// fired
...
/// When then raw appending is done. The events are fired that the document has
/// been changed The undo-collection is enabled again
...
/// @param line the line number which contains this offset. (When -1 the line
