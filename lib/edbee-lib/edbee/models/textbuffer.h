...
{
public:
  TextBufferChangeData(TextBuffer *buffer, int off, int len, const QChar *text,
                       int textlen);
  TextBufferChangeData(LineOffsetVector *lineOffsets, int off, int len,
...
/// This clas represents a text buffer change and is used to pass around between
/// events This is a shareddata object so the data can be thrown between
/// different threads (delayed emit-support)_
/// TODO: Still problematic maybe the QChar* text pointer. It is possible that
/// this pointer is being freed.
class EDBEE_EXPORT TextBufferChange
{
public:
  TextBufferChange();
  TextBufferChange(TextBuffer *buffer, int off, int len, const QChar *text,
                   int textlen);
  TextBufferChange(LineOffsetVector *lineOffsets, int off, int len,
                   const QChar *text, int textlen);
...
public:
  ...
  virtual void replaceText(int offset, int length, const QChar *buffer,
                           int bufferLength)
      = 0;

  /// this method should return an array with all line offsets. A line offset
  /// pointsto the START of a line So it does NOT point to a newline character,
  /// but it points to the first character AFTER the newline character
  ...
  /// the end raw append method should bring the document in a consistent state
  /// and emit the correct "replaceText" signals
  ///
  /// WARNING the textAboutToBeReplaced signals are given but at that moment the
  /// text is already replaced And the newlines are already added to the newline
  /// list!
  virtual void rawAppendEnd() = 0;

  /// This method returns the raw data buffer.
  /// WARNING this method CAN be slow because when using a gapvector the gap is
  /// moved to the end to make a full buffer Modifying the content of the data
  /// will mess up the line-offset-vector and other dependent classes. For
  /// reading it's ok :-)
  ...
  virtual int findCharPos(int offset, int direction, const QString &chars,
                          bool equals);
  virtual int findCharPosWithinRange(int offset, int direction,
                                     const QString &chars, bool equals,
                                     int beginRange, int endRange);
  virtual int findCharPosOrClamp(int offset, int direction,
                                 const QString &chars, bool equals);
  virtual int findCharPosWithinRangeOrClamp(int offset, int direction,
                                            const QString &chars, bool equals,
                                            int beginRange, int endRange);
