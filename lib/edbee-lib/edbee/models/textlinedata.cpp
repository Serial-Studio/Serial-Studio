...
TextLineDataList::~TextLineDataList()
{
  if (lineDataList_)
  {
    //        Q_ASSERT(false);    /// YOU MUST call destroy before the
    //        destructor!
    qlog_warn()
        << "** Warning TextLineDataList requires manual destruction! ** ";
  }
}
...
void TextLineDataList::give(TextLineDataManager *manager, int field,
...
/// This method returns the given data item and transfers the ownership. The
/// item in the list is set to 0
...
void TextLineDataList::realloc(TextLineDataManager *manager,
                               int oldFieldsPerLine, int newFieldsPerLine)
{
  // qlog_info() << "realloc:" << oldFieldsPerLine << " >> " <<
  // newFieldsPerLine;
...
void TextLineDataManager::give(int line, int field, TextLineData *dataItem)
{
  ...
  if (!list)
  {
    textLineDataList_.set(line, list = new TextLineDataList());
  }
...
TextLineData *TextLineDataManager::get(int line, int field)
{
  ...
  if (list)
  {
    return list->at(this, field);
  }
  return 0;
}

/// This method is called to notify that some lines have been replaced
// void TextLineDataManager::linesReplaced(int lineStart, int lineCount, int
// newLineCount)
//{
//     Q_ASSERT(false);
...
//{
//     qlog_info() << "GAPVECTOR: ["<<textLineDataList_.gapBegin() <<
//     ","<<textLineDataList_.gapEnd()<<">"; for( int i=0; i <
//     textLineDataList_.capacity(); ++i ) {
//         TextLineDataList* item = textLineDataList_.rawAt(i);
//         qlog_info() << "-" << i <<  ":" <<
//         QStringLiteral("%1").arg((quintptr)item);
//     }
//     qlog_info()<<"- done";
...
/// 'PredefinedFieldCount' amont of fields are required
...
Change *TextLineDataManager::createLinesReplacedChange(int lineStart,
                                                       int lineCount,
                                                       int newLineCount)
{
  // no changes
  if (lineCount == 0 && lineCount == newLineCount)
  {
    return 0;
  }

  LineDataListChange *change
      = new LineDataListChange(this, lineStart, lineCount, newLineCount);
  return change;
}
...
void TextLineDataManager::fillWithEmpty(int line, int length, int newLength)
{
  ...
  // qlog_info() << "- TextLineDataManager::fillWithEmpty( line: " << line << ",
  // length " << length << " newLength: " << newLength << ") len:" <<
  // this->textLineDataList_.length();
  emit lineDataChanged(line, length, newLength);
}

/// Replace the given items with
/// emits a lineDataChanged signal
void TextLineDataManager::replace(int line, int length,
                                  TextLineDataList **items, int newLength)
{
  destroyRange(line, length);
