...
/// Sets the anchor to the given location, and forces the anchor to say between
/// the document bounds
...
/// Sets the caret to the given location, and forces the caret to say between
/// the document bounds
...
/// This method converts a text-selection range to a string helpfull with
/// debuggin
...
/// @param amount the amount to move
int TextRange::moveWhileChar(TextDocument *doc, int pos, int amount,
                             const QString &chars)
{
  ...
  {
    --pos; // first move left
    while (pos >= 0 && chars.indexOf(doc->charAt(pos)) >= 0)
    {
      --pos;
    }
    ++pos;
...
int TextRange::moveUntilChar(TextDocument *doc, int pos, int amount,
                             const QString &chars)
{
  int docLength = doc->length();
  if (amount < 0)
...
void TextRange::moveCaretWhileChar(TextDocument *doc, int amount,
                                   const QString &chars)
{
  caret_ = moveWhileChar(doc, caret_, amount, chars);
}

void TextRange::moveCaretUntilChar(TextDocument *doc, int amount,
                                   const QString &chars)
{
  caret_ = moveUntilChar(doc, caret_, amount, chars);
}

void TextRange::moveAnchortWhileChar(TextDocument *doc, int amount,
                                     const QString &chars)
{
  anchor_ = moveWhileChar(doc, anchor_, amount, chars);
}

void TextRange::moveAnchorUntilChar(TextDocument *doc, int amount,
                                    const QString &chars)
{
  anchor_ = moveWhileChar(doc, anchor_, amount, chars);
}
...
void TextRange::moveCaretByCharGroup(TextDocument *doc, int amount,
...
/// @par    am whitespace the characters that need to be interpreted as
/// whitespace
void TextRange::moveCaretToLineBoundary(TextDocument *doc, int amount,
                                        const QString &whitespace)
{
  ...
  if (amount < 0)
  {
    ...
    int wordStart = buf->findCharPosWithinRangeOrClamp(
        lineStart, 1, whitespace, false, lineStart, offsetNextLine);
...
void TextRange::moveCaretToWordBoundaryAtOffset(TextDocument *doc,
                                                int newOffset)
{
...
/// Moves the caret to a word boundary  (used for word dragging selections)
void TextRange::moveCaretToLineBoundaryAtOffset(TextDocument *doc,
                                                int newOffset)
{
...
/// 0 is a special case, it moves the caret to the start of the current line and
/// expands to the end of the line. It does not add lines
...
static QString findCharGroup(QChar c, const QString &whitespace,
                             const QStringList &characterGroups)
{
  QString foundGroup;
                             ...
                             const QStringList &characterGroups)
{
  ...
  {
    QString group
        = findCharGroup(doc->charAt(min - 1), whitespace, characterGroups);
    if (group.isEmpty())
    {
  ...
  {
    QString group
        = findCharGroup(doc->charAt(max), whitespace, characterGroups);
    if (group.isEmpty())
    {
      group = characterGroups.join("").append(whitespace); // the 'else' group
...
/// Checks if two ranges touch eachother. Touching means that the start and end
/// of two ranges are onto eachother Possible situations ( [ = anchor, > =
/// caret, ( = don't care )
///  (A)(B) or (B)(A)
...
/// returns the range indices that are being overlapped by the given offsetBegin
/// and offsetEnd
...
bool TextRangeSetBase::rangesBetweenOffsets(int offsetBegin, int offsetEnd,
                                            int &firstIndex, int &lastIndex)
{
  ...
  for (int i = 0, cnt = rangeCount(); i < cnt; ++i)
  {
    ...
    if ((offsetBegin <= minOffset && minOffset <= offsetEnd)
        || (minOffset <= offsetBegin && offsetBegin <= maxOffset))
    {
      if (firstIndex < 0)
        firstIndex = i;
      lastIndex = i;
...
/// @param lastIndex(out) The last index found (-1 if not found)
/// @return true if the range is found
bool TextRangeSetBase::rangesBetweenOffsetsExlusiveEnd(int offsetBegin,
                                                       int offsetEnd,
                                                       int &firstIndex,
                                                       int &lastIndex)
{
...
/// Returns the range indices that are being used on the given line (Excluding
/// the last offset)
bool TextRangeSetBase::rangesAtLineExclusiveEnd(int line, int &firstIndex,
                                                int &lastIndex)
{
  TextDocument *doc = textDocument();
  int offsetBegin = doc->offsetFromLine(line);
  int offsetEnd = doc->offsetFromLine(line + 1) - 1;
  return rangesBetweenOffsetsExlusiveEnd(offsetBegin, offsetEnd, firstIndex,
                                         lastIndex);
}
...
{
  if (sel.rangeCount() != rangeCount())
    return false;
  for (int i = rangeCount() - 1; i >= 0; --i)
  {
    if (!range(i).equals(sel.range(i)))
      return false;
  }
  return true;
}
...
/// This method converts the selection ranges as a string, in the format:
...
/// WARNING, you should ONLY call this method if the operation you performed
/// kept the rangeset in a valid state. This (at least) means the ranges need to
/// be sorted
...
void TextRangeSetBase::expandToWords(const QString &whitespace,
                                     const QStringList &characterGroups)
{
  for (int i = 0, cnt = rangeCount(); i < cnt; ++i)
  {
...
void TextRangeSetBase::selectWordAt(int offset, const QString &whitespace,
                                    const QStringList &characterGroups)
{
...
/// The idea is the following, double-click an empty place to select the word at
/// the given location Double click an existing selection to remove the
/// selection (and caret)
void TextRangeSetBase::toggleWordSelectionAt(int offset,
                                             const QString &whitespace,
                                             const QStringList &characterGroups)
{
...
void TextRangeSetBase::moveCaretsByCharGroup(int amount,
                                             const QString &whitespace,
                                             const QStringList &characterGroups)
{
  for (int rangeIdx = rangeCount() - 1; rangeIdx >= 0; --rangeIdx)
  {
    range(rangeIdx).moveCaretByCharGroup(textDocument(), amount, whitespace,
                                         characterGroups);
  }
  processChangesIfRequired();
}

/// Moves all carets to the given line boundary (line-boundary automatically
/// switches between column 0 and first non-whitespace character)
/// @param direction the direction < 0 to the start of the line (or first char)
/// > 0 to the end of the line
/// @param whitespace the characters to see as whitespace
void TextRangeSetBase::moveCaretsToLineBoundary(int direction,
                                                const QString &whitespace)
{
  // process all carets
  for (int rangeIdx = rangeCount() - 1; rangeIdx >= 0; --rangeIdx)
  {
    range(rangeIdx).moveCaretToLineBoundary(textDocument(), direction,
                                            whitespace);
  }
  processChangesIfRequired();
}
...
/// @param joinBorders if joinborders is set, borders next to eachother are also
/// interpretted as overlap. (inclusive/exclusive switch)
...
/// @param index the index of the range to change (when not given index 0 is
/// assumed)
...
/// This method adds (or removes) the given spatial length at the given
/// location.
///
/// It adjusts all locations, anchors with the given locations. It automatically
/// moves carets, 'removes' selection etc.
///
...
/// @param sticky, when sticky the caret/anchor is sticky and isn't moved if the
/// change happens at the same location
void TextRangeSetBase::changeSpatial(int pos, int length, int newLength,
                                     bool sticky, bool performDelete)
{
...
{
  std::sort(selectionRanges_.begin(), selectionRanges_.end(),
            TextRange::lessThan);
}
...
DynamicTextRangeSet::DynamicTextRangeSet(TextDocument *doc, bool stickyMode,
                                         bool deleteMode, QObject *parent)
  : QObject(parent)
  , TextRangeSet(doc)
  , stickyMode_(stickyMode)
  , deleteMode_(deleteMode)
{
  connect(textDocument(), &TextDocument::textChanged, this,
          &DynamicTextRangeSet::textChanged);
}
...
/// @param stickyMode is the stickymode enabled
/// @param parent the parent owner
DynamicTextRangeSet::DynamicTextRangeSet(const TextRangeSet &sel,
                                         bool stickyMode, bool deleteMode,
                                         QObject *parent)
  : QObject(parent)
  , TextRangeSet(sel)
  , stickyMode_(stickyMode)
...
{
  Q_UNUSED(oldText)
  changeSpatial(change.offset(), change.length(), change.newTextLength(),
                stickyMode_, deleteMode_);
}
