  ...
  , autoCompleteProviderList_(0)
{
  ...
  connect(textBuffer_, SIGNAL(textAboutToBeChanged(edbee::TextBufferChange)),
          SIGNAL(textAboutToBeChanged(edbee::TextBufferChange)),
          Qt::DirectConnection);
  connect(textBuffer_, SIGNAL(textChanged(edbee::TextBufferChange, QString)),
          SLOT(textBufferChanged(edbee::TextBufferChange, QString)),
          Qt::DirectConnection);

  // forward the persisted state changes
  connect(textUndoStack_, SIGNAL(persistedChanged(bool)), this,
          SIGNAL(persistedChanged(bool)));

  connect(textScopes_, SIGNAL(lastScopedOffsetChanged(int, int)), this,
          SIGNAL(lastScopedOffsetChanged(int, int)));
}
...
Change *CharTextDocument::giveChangeWithoutFilter(Change *change,
                                                  int coalesceId)
{
...
// void CharTextDocument::replaceTextWithoutFilter( int offset, int length,
// const QString& text )
//{
//     SingleTextChange *textChange = new SingleTextChange( offset, length,
//     text); textUndoStack()->giveAndExecuteChange( textChange, true );
// }
                                         ...
                                         QString oldText)
{
  ...
  {
    Change *lineDataChange = lineDataManager()->createLinesReplacedChange(
        change.line() + 1, change.lineCount(), change.newLineCount());
    if (lineDataChange)
    {
