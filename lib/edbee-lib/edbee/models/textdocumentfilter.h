...
public:
  ...
  /*
      ...
      virtual void filterBeginGroup( TextDocument* doc, TextChangeGroup* group )
     = 0;

      /// This method is called when group is ended
      virtual void filterEndGroup( TextDocument* doc, TextChangeGroup* group,
     int coalesceId, bool flatten ) = 0;
  */
  ...
  virtual void filterReplaceRangeSet(TextDocument *doc, TextRangeSet &rangeSet,
                                     QString &str);

  /// makes it possible to modify the range-set string that's passed to the
  /// document
  /// @param doc the textdocument
  /// @param rangeSet the rangeSet that's replaced
  ...
  ///
  /// You can do what you want with this text change. Just remember you are the
  /// owner and if you do nothing. Nothing happens
  ///
  /// WARNING: When you do NOT use the changes object you should delete it!!!
  ///
  /// When done you can call doc->giveTextChangeWithoutFilter( changes ) for
  ...
  virtual Change *filterChange(TextDocument *doc, Change *change,
                               int coalesceId)
      = 0;
};
