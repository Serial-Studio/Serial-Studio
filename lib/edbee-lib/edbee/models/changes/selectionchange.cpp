...
void SelectionChange::execute(TextDocument *document)
{
  ...
  TextRangeSet *currentSelection
      = dynamic_cast<TextRangeSet *>(controllerContext()->textSelection());
...
{
  return QStringLiteral("SelectionTextChange(%1)")
      .arg(rangeSet_->rangesAsString());
}

/// This method is called internally for notifying the control the selection has
/// been changed Perhaps we should make e proper emit-signal for this purpose
void SelectionChange::notifyChange()
{
  /// TODO: make the controllerContext only repaint the affected areas via the
  /// TextRangeSets
  controllerContext()->onSelectionChanged(rangeSet_);

  AccessibleTextEditorWidget::notifyTextSelectionEvent(
      controller()->widget(), controller()->textSelection());
}
