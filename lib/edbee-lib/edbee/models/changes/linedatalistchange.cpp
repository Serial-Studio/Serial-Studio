...
LineDataListChange::LineDataListChange(TextLineDataManager *manager, int line,
...
void LineDataListChange::execute(TextDocument *document)
{
  ...
  for (int i = 0; i < contentLength_; ++i)
  {
    TextLineDataList *list
        = managerRef_->takeList(offset_ + i); // take the ownership of the list
    if (list)
    {
      // jit allocation of the list!
      if (!oldListList_)
      {
        oldListList_ = new TextLineDataList
            *[contentLength_](); /// new X[]() => fills it with zero's
      }
      oldListList_[i] = list;
    }
  }
...
{
  LineDataListChange *lineTextChange
  ...
  int newOldListSize = getMergedStoredLength(
      change); // qlog_info() << "CALCULATED: " << newOldListSize ;
  ...
  // merge the stuff
  mergeStoredDataViaMemcopy(newOldListList_, oldListList_,
                            lineTextChange->oldListList_, change,
                            sizeof(TextLineDataList *));
...
bool LineDataListChange::giveAndMerge(TextDocument *document,
                                      Change *textChange)
{
  Q_UNUSED(document);
  Q_UNUSED(textChange);
...
{
  return QStringLiteral("LineDataListTextChange(%1,%2,%3)")
      .arg(offset_)
      .arg(contentLength_)
      .arg(docLength_);
}
