...
/// A full line data text change. This means the growing or shrinking of the
/// line data buffer It stores the old-data list that needs to be remebered for
/// undoing
class EDBEE_EXPORT LineDataListChange : public AbstractRangedChange
{
public:
  LineDataListChange(TextLineDataManager *manager, int offset, int lenght,
                     int newLength);
