...
/// @param executed, a boolean (mainly used for testing) to mark this change as
/// exected
...
void TextChange::mergeStoredData(AbstractRangedChange *change)
{
  ...
  mergeStoredDataViaMemcopy(newText.data(), text_.data(),
                            singleTextChange->text_.data(), change,
                            sizeof(QChar));
  /*
    QString newText;
    // we first need to 'take' the leading part of the new change
    if( change->offset() < offset() ) {
        newText.append( change->oldText(document).mid(0, offset() -
    change->offset() ) );
    }
...
/// This method gives the given change to this textchange. The changes will be
/// merged if possible. This method currently only works with executed
/// changes!!!
///
