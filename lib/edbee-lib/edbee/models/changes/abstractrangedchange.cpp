...
/// This method merges the data via a memcopy. I really don't like this way of
/// merging the data. though this is a way which support all kinds of arrays. I
/// cannot use inheritance, because the types like QString don't have a common
/// ancestor
///
/// @param targetData the target of the data, (be sure enough space is
/// reserved!!
/// @param data pointer to the data of this change (this can be a 0 pointer!,
/// which results in 0-filling the target)
/// @param changeData pointer to the of the other change data (this can be a 0
/// pointer!, which results in 0-filling the target)
/// @param change the other change of the data to merge
/// @param itemSize the size of single item
void AbstractRangedChange::mergeStoredDataViaMemcopy(
    void *targetData, void *data, void *changeData,
    AbstractRangedChange *change, int itemSize)
{
  ...
  if (0 <= remainerOffset && remainerOffset < change->storedLength())
  {
    memcopy_or_zerofill(
        target,
        changeData ? ((char *)changeData) + (remainerOffset * itemSize) : 0,
        (change->storedLength() - remainerOffset) * itemSize);
  }
}
...
/// overlapping is an exclusive overlap, which means the changes are really on
/// top of eachother to test if the changes are touching use isTouchedBy
...
bool AbstractRangedChange::isOverlappedBy(AbstractRangedChange *secondChange)
{
  return (offset() < (secondChange->offset() + secondChange->docLength())
          && secondChange->offset() < (offset() + docLength()))
         || (offset() < (secondChange->offset() + secondChange->storedLength())
             && secondChange->offset() < (offset() + docLength()));
}

/// Touched ranges are ranges that are next to eachother
/// Touching means the end offset of one range is the start offset of the other
/// range
...
bool AbstractRangedChange::isTouchedBy(AbstractRangedChange *secondChange)
{
  return offset() == (secondChange->offset() + secondChange->docLength())
         ...
         || (secondChange->docLength() < secondChange->storedLength()
             && offset()
                    == (secondChange->offset() + secondChange->storedLength()))
      // delete touch (Should we add those length < newlength condition!??)
      //|| ( ( length() < newLength() ) && (offset() + newLength()) ==
      //secondChange->offset() )
      ;
}
