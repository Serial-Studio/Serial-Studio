...
/// This are changes (text changes and line changes) that span a range in an
/// array. These ranges share a common alogrithm for performing merges,
/// detecting overlaps etc.
...
protected:
  ...
  /// dynamic_cast<SingleTextChange*>(change); QString newText; newText.resize(
  /// calculateMergeDataSize( change) ); mergeData( newText.data(),
  /// text_.data(), singleTextChange->text_.data(), change, sizeof(QChar) );
  ...
  void mergeStoredDataViaMemcopy(void *targetData, void *data, void *changeData,
