...
void MergableChangeGroup::execute(TextDocument *document)
{
  ...
  if (newSelection_)
  {
    TextRangeSet *currentSelection
        = dynamic_cast<TextRangeSet *>(controller()->textSelection());
...
void MergableChangeGroup::addOffsetDeltaToChanges(
    QList<AbstractRangedChange *> &changes, int fromIndex, int delta)
{
...
int MergableChangeGroup::findInsertIndexForOffset(
    QList<AbstractRangedChange *> &changes, int offset)
{
  int insertIndex = 0;
...
int MergableChangeGroup::mergeChange(QList<AbstractRangedChange *> &changes,
                                     TextDocument *doc,
                                     AbstractRangedChange *newChange,
                                     int &delta)
{
  for (int i = 0, cnt = changes.size(); i < cnt; ++i)
  {
    ...
    // we need the previous length and new-length to know how the delta is
    // changed of the other items
    ...
    if (change->giveAndMerge(doc, newChange))
    {
      ...
      delta += (change->docLength() - prevNewLength)
               - (change->storedLength() - prevContentLength);
      return i;
    }
  }
  return -1;
...
/// When a normal merge was succesful (with mergeTextChange) we need to
/// inverseMerge the other overlapping textchanges
...
void MergableChangeGroup::inverseMergeRemainingOverlappingChanges(
    QList<AbstractRangedChange *> &changes, TextDocument *doc,
    int mergedAtIndex, int orgStartOffset, int orgEndOffset, int delta)
{
  ...
  for (int i = mergedAtIndex + 1; i < changes.size(); ++i)
  {
    ...
    if (nextChange->offset() < orgEndOffset
        && orgStartOffset < (nextChange->offset() + nextChange->docLength()))
    {

      // take the delta of the previous change before the merge
      int tmpDelta
          = mergedChange->storedLength() - mergedChange->docLength() + delta;

      // alter the delta, so we find the correct merge index
      nextChange->addOffset(tmpDelta);

      // notice the 'inversion of the merge. We apply the merged change to the
      // next change
...
void MergableChangeGroup::giveChangeToList(
    QList<AbstractRangedChange *> &changes, TextDocument *doc,
    AbstractRangedChange *newChange)
{
  // qlog_info() << "giveSingleTextChange" << newChange->toString();
  //  remember the orginal ranges so we know which changes are affected by this
  //  new change
  int orgStartOffset = newChange->offset();
  int orgEndOffset = newChange->offset() + newChange->storedLength();

  // some variables to remebmer
  int addDeltaFromIndex
      = size(); // From which change index should we add delta?!
  int delta = 0;
  ...
  if (mergedAtIndex >= 0)
  {
    ...
    inverseMergeRemainingOverlappingChanges(
        changes, doc, mergedAtIndex, orgStartOffset, orgEndOffset, delta);
...
void MergableChangeGroup::giveSingleTextChange(TextDocument *doc,
                                               TextChange *change)
{
...
void MergableChangeGroup::giveLineDataListTextChange(
    TextDocument *doc, LineDataListChange *newChange)
{
  Q_UNUSED(doc);
  this->lineDataTextChangeList_.append(newChange); // no merge for the moment
  /* TODO:  Work in progress below. This algorithm seems identical to the one
   used with SingleTextChanges
   *  we need to abstract the interfaces and create a general merge-ranges
   operation
   *
      ...
      for( int i=0, cnt=lineDataTextChangeList_.size(); i<cnt; ++i ) {
          ...
          // we need the previous length and new-length to know how the delta is
   changed of the other items int prevLength = change->length(); int
   prevNewLength = change->newLength();

          if( change->giveAndMerge( doc, newChange ) ) {
              mergeIndex = i;
              delta += (change->length()-prevLength) -
   (change->newLength()-prevNewLength); break;
          }
      }
...
void MergableChangeGroup::giveChange(TextDocument *doc, Change *change)
{
  ...
  // a list text change
  LineDataListChange *lineDataTextChange
      = dynamic_cast<LineDataListChange *>(change);
  ...
  if (selectionChange)
  {
    ...
    /// we can simply delete the change, the ComplexTextChange automatically
    /// records the last change selection on the undoGroupEnd
    delete change;
    return;
  }
  ...
  // other changes are (currently) added to a misch change list. And are pretty
  // scary for now :)
...
{
  return textChangeList_.size() + lineDataTextChangeList_.size()
         + miscChangeList_.size();
}
...
bool MergableChangeGroup::giveAndMerge(TextDocument *document,
                                       Change *textChange)
{
  ...
  foreach (AbstractRangedChange *abstractChange, textChangeList_)
  {
    ...
    if (change)
    {
      ...
      result.append(QStringLiteral("%1:%2:%3")
                        .arg(change->offset())
                        .arg(change->docLength())
                        .arg(change->storedText()));
    }
  }
  return result;
}
...
void MergableChangeGroup::moveChangesFromGroup(TextDocument *doc,
                                               ChangeGroup *group)
{
  ...
  // next we need to compress the changes (merging changes that are next to
  // eachother)
  compressChanges(doc);
  // qlog_info() << "************************";
  ...
  MergableChangeGroup *complexGroup
      = dynamic_cast<MergableChangeGroup *>(group);
  if (complexGroup)
  {
...
bool MergableChangeGroup::mergeAsGroup(TextDocument *document,
                                       Change *textChange)
{
  Q_UNUSED(document);
...
/// @param textChange the textchange to merge
/// @return true if the textchange was merged as group.
bool MergableChangeGroup::mergeAsSelection(TextDocument *document,
                                           Change *textChange)
{
  Q_UNUSED(document);
  SelectionChange *selectionChange
      = dynamic_cast<SelectionChange *>(textChange);
  if (!selectionChange)
  {
    return false;
  }

  // simply take the selection from the textchange and make this the new
  // selection
  delete newSelection_;
  newSelection_ = selectionChange->takeRangeSet();
  delete textChange;
...
void MergableChangeGroup::compressTextChanges(TextDocument *document)
{
  ...
  for (int i = 0; i < textChangeList_.size(); ++i)
  {
    ...
    for (int j = i + 1; j < textChangeList_.size(); ++j)
    {
      ...
      {
        textChangeList_.takeAt(
            j); // just take it, the item is already deleted by the merge
        --j;
      }
    } // for j
  } // for i
