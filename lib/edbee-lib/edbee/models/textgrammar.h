...
public:
  ...
  enum Instruction
  {
    ...
    Parser ///< A full parser (not yet supported, by added as idea for the
           ///< future). But could be marked by multiple regexps
  };

  TextGrammarRule(TextGrammar *grammar, Instruction instruction);
  ~TextGrammarRule();

  static TextGrammarRule *createMainRule(TextGrammar *grammar,
                                         const QString &scopeName);
  static TextGrammarRule *createRuleList(TextGrammar *grammar);
  static TextGrammarRule *createSingleLineRegExp(TextGrammar *grammar,
                                                 const QString &scopeName,
                                                 const QString &regExp);
  static TextGrammarRule *createMultiLineRegExp(TextGrammar *grammar,
                                                const QString &scopeName,
                                                const QString &contentScopeName,
                                                const QString &beginRegExp,
                                                const QString &endRegExp);
  static TextGrammarRule *createIncludeRule(TextGrammar *grammar,
  ...
  {
    matchCaptures_.insert(idx, name);
  }
  void setEndCapture(int idx, const QString &name)
  {
    endCaptures_.insert(idx, name);
  }
...
private:
  ...
  // RegExp* endRegExp_;                  ///< The end regular expression
  // matcher
  ...
  QMap<int, QString>
      endCaptures_; ///< all end captures that need to be performed
...
public:
  ...
  TextGrammarRule *findFromRepos(const QString &name,
...
private:
  ...
  QMap<QString, TextGrammarRule *>
      repository_;             ///< A map with all named grammar rules
...
private:
  ...
  QMap<QString, TextGrammar *>
      grammarMap_;           ///< A map with all grammar definitions
