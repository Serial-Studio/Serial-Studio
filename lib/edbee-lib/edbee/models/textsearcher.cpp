...
/// This method does not alter the textrange selection. It only returns the
/// range of the next match
/// @param selection the text-selection to use
/// @return the textRange with the found text.  TextRange::isEmpty() can be
/// called to check if nothing has been found
...
/// Selects the next item that matches the given critaria (Adds an extra
/// selection range )
...
bool TextSearcher::selectNext(TextRangeSet *selection)
{
  ...
  if (!range.isEmpty())
  {
    ...
    // edge case, when currently ony 1 caret is available and doesn't have a
    // selection we need to remove this range
...
{
  std::unique_ptr<TextRangeSet> newRangeSet(
      new TextRangeSet(widget->textSelection()));
...
/// A smart selection: when there's no selection the current word is used and
/// selected. The next word will be selected, if all is specified all words are
/// selected
/// @param widget the widget to select
/// @param selectAllTexts if true then all occurences are selected
void TextSearcher::selectUnderExpand(TextEditorWidget *widget,
                                     bool selectAllTexts)
{
  std::unique_ptr<TextRangeSet> newRangeSet(
      new TextRangeSet(widget->textSelection()));
  ...
  if (!newRangeSet->hasSelection() || newRangeSet->rangeCount() == 1)
  {
    ...
    if (!newRangeSet->hasSelection())
    {
      if (range.isEmpty())
      {
        ...
        runSelect
            = selectAllTexts; // only run select when we need to select all
      }
    }
...
RegExp *TextSearcher::createRegExp()
{
  ...
  if (syntax() == SyntaxRegExp)
    regExpSyntax = RegExp::SyntaxDefault;

  RegExp *regExp = new RegExp(searchTerm(), caseSensitive_, regExpSyntax,
                              RegExp::EngineOniguruma);
  return regExp;
}
