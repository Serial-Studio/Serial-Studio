...
//{
//     QMap<QKeySequence,QString>::const_iterator i =
//     keyMap.actionMap_.constBegin(); while (i != keyMap.actionMap_.constEnd())
//     {
//         set( i.key(), i.value() );
//     }
...
{
  for (QMultiHash<QString, TextEditorKey *>::const_iterator itr
       = keyMap_.constBegin();
       itr != keyMap_.constEnd(); ++itr)
  {
...
/// If this doesn't result in a sequence it assumes it's a keydefinition used by
/// QKeySequence
...
bool TextEditorKeyMap::add(const QString &command, const QString &seq)
{
  ...
  QKeySequence::StandardKey standardKey
      = TextEditorKeyMap::standardKeyFromString(seq);
...
// void TextEditorKeyMap::set(const QString& name, const
...
/// Warning when one keysequence is the prefix of the other keysequence it is
/// possible only the partial or full match is return (depending on the order)
///
...
QString TextEditorKeyMap::findBySequence(QKeySequence sequence,
                                         QKeySequence::SequenceMatch &match)
{
  ...
  if (parentRef_)
    return parentRef_->findBySequence(sequence, match);
...
QKeySequence TextEditorKeyMap::joinKeySequences(const QKeySequence seq1,
                                                const QKeySequence seq2)
{
  if (seq1.count() + seq2.count() > 4)
  {
    return seq2;
  }
  int idx = 0;
...
TextEditorKeyMap::standardKeyFromString(const QString &str)
{
  ...
  if (h.isEmpty())
  {
    ...
    h.insert("insert_paragraph_separator",
...
QString TextEditorKeyMap::toString() const
{
       ...
       itr != keyMap_.constEnd(); ++itr)
  {
    if (!str.isEmpty())
      str.append(",");
    str.append(QStringLiteral("%1:%2").arg(itr.key()).arg(
        itr.value()->sequence().toString()));
  }
  return str;
}
...
void TextKeyMapManager::loadKeyMap(const QString &file)
{
  ...
  {
    qlog_warn() << QObject::tr("Error parsing %1: %2 ")
                       .arg(file)
    ...
    {
      qlog_warn()
          << "Error loading default keymap: fallback to the factory keymap!";
      loadFactoryKeyMap();
    }
  }
}
