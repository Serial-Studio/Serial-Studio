...
MultiLineScopedTextRangeReference::MultiLineScopedTextRangeReference(
    MultiLineScopedTextRange &range)
...
MultiLineScopedTextRangeReference::multiLineScopedTextRange()
{
...
/// set the independent flag. The independent flag means it's not dependent on
/// other lines
...
bool MultiLineScopedTextRange::lessThan(MultiLineScopedTextRange *r1,
                                        MultiLineScopedTextRange *r2)
{
  ...
  if (diff == 0)
  {
    diff
        = r2->length()
          - r1->length(); // larger areas need to be placed before smaller areas
  }
  return (diff < 0);
}
...
/// You usually don't need to supply the scopeManager parameter. It's only
/// required on application startup
///
/// @param fullScope the name of the scope
/// @param scopeManager the scopemanager to use (when 0 this defaults to the
/// global edbee scopemanager)
...
bool TextScope::startsWith(TextScope *scope)
{
  ...
  for (int i = 0; i < scope->scopeAtomCount_; ++i)
  {
    ...
    bool atomEqual = atomId == scopeAtomId || atomId == wildCard
                     || scopeAtomId == wildCard;
    if (!atomEqual)
    {
      return false;
    }
...
/// This method returns the 'index' of the given full-scope. This is a kind of
/// substring search
///
...
TextScopeSelector::TextScopeSelector(const QString &selector)
{
  ...
  {
    sel = sel.trimmed();
    selectorList_.append(
        Edbee::instance()->scopeManager()->createTextScopeList(sel));
  }
}
...
/// Currently the scope-calculation is very 'basic'. Just enough to perform the
/// most basic form of matching
///
/// The algorithm returns 1.0 on a full match...
///
///
/// @param scopeList the list of scopes
/// @return the matching score. -1 means no match found. >= 0 a match has been
/// found
...
/// @return the matching score. -1 means no match found. >= 0 a match has been
/// found
double TextScopeSelector::calculateMatchScoreForSelector(
    TextScopeList *selector, const TextScopeList *scopeList)
{
  double result = 0.0;
  double power = 0.0;
...
TextScopeManager::~TextScopeManager()
{
  foreach (TextScope *textScope, textScopeList_)
  {
    delete textScope;
  }
...
/// This method clears and delets all scopes. WARNING this destroys all
/// registered text-scopes This method also registers the wildcard scope atom id
...
TextScopeList *
TextScopeManager::createTextScopeList(const QString &scopeListString)
{
...
MultiLineScopedTextRangeSet::MultiLineScopedTextRangeSet(
    TextDocument *textDocument, TextDocumentScopes *textDocumentScopes)
...
{
  scopedRangeList_.append(new MultiLineScopedTextRange(
      anchor, caret, Edbee::instance()->scopeManager()->refEmptyScope()));
}
...
{
  std::sort(scopedRangeList_.begin(), scopedRangeList_.end(),
            MultiLineScopedTextRange::lessThan);
}
...
MultiLineScopedTextRange &MultiLineScopedTextRangeSet::addRange(
    int anchor, int caret, const QString &name, TextGrammarRule *rule)
{
  MultiLineScopedTextRange *tr = new MultiLineScopedTextRange(
      anchor, caret, Edbee::instance()->scopeManager()->refTextScope(name));
  tr->setGrammarRule(rule);
  scopedRangeList_.append(tr);
  return *tr;
}

/// This method removes all ranges after a given offset. This means it will
/// remove all complete ranges after the given offset. Ranges that start before
/// the offset and end after the offset are 'invalidated' which means the end
/// offset is placed to the end of the document
void MultiLineScopedTextRangeSet::removeAndInvalidateRangesAfterOffset(
    int offset)
{
  ...
  endChangesWithoutProcessing(); // we only deleted the last range. Do the
                                 // result is still sorted
}

/// This method gives the scoped text range to this object
void MultiLineScopedTextRangeSet::giveScopedTextRange(
    MultiLineScopedTextRange *textScope)
{
...
TextDocumentScopes::TextDocumentScopes(TextDocument *textDocument)
  ...
  , defaultScopedRange_(
        0, 0, Edbee::instance()->scopeManager()->refTextScope("text.plain"))
  , scopedRanges_(textDocument, this)
  , lastScopedOffset_(0)
{
  connect(textDocument, SIGNAL(languageGrammarChanged()), this,
          SLOT(grammarChanged()));
}
...
void TextDocumentScopes::setDefaultScope(const QString &name,
                                         TextGrammarRule *rule)
{
  defaultScopedRange_.setScope(
      Edbee::instance()->scopeManager()->refTextScope(name));
...
void TextDocumentScopes::giveLineScopedRangeList(int line,
...
{
  if (line >= lineRangeList_.length() || line < 0)
  {
    return 0;
  }
...
void TextDocumentScopes::giveMultiLineScopedTextRange(
    MultiLineScopedTextRange *range)
{
...
/// Currently this is done very dirty, by retrieving the defaultscoped range the
/// begin and end is set tot he complete document a better solution would be a
/// subclass that always returns 0 for an anchor and the documentlength for the
/// caret
MultiLineScopedTextRange &TextDocumentScopes::defaultScopedRange()
{
  defaultScopedRange_.set(
      0, textDocument()->length()); // make sure this is always set like this
  return defaultScopedRange_;
}

/// This method returns all scope-ranges at the given offset-ranges
QVector<MultiLineScopedTextRange *>
                                                        ...
                                                        int offsetEnd)
{
  ...
  int minOffset = 0;
  for (int i = 0, cnt = scopedRanges_.rangeCount();
       i < cnt && minOffset <= /** CHANGED 2013-01-22, was < */ offsetEnd; ++i)
  {
...
TextScopeList TextDocumentScopes::scopesAtOffset(int offset, bool includeEnd)
{
  ...
  // QVector<MultiLineScopedTextRange*> ranges =
  // scopedRangesBetweenOffsets(offsetBegin, offsetEnd);
  ...
  if (list)
  {
    ...
    for (int i = 0, cnt = list->size(); i < cnt; ++i)
    {
      ...
      // QString debug;
      // debug.append( QStringLiteral("- %1.%5:
      // %2<=%3<%4").arg(i).arg(range->min()).arg(offsetInLine).arg(range->max()).arg(range->scope()->name())
      // );
      if (range->min() <= offsetInLine)
      {
        if (offsetInLine < range->max()
            || (includeEnd && offsetInLine <= range->max()))
        {
          // debug.append(" Ok" );
...
/// Hmmm this is almost exactly the same implementation as the scopesAtOffset
/// method !? (perhaps we should refactor this)
///
/// Warning you MUST destroy (qDeleteAll) the list with scoped textranges
...
QVector<ScopedTextRange *>
TextDocumentScopes::createScopedRangesAtOffsetList(int offset)
{
  ...
  if (list)
  {
    ...
    for (int i = 0, cnt = list->size(); i < cnt; ++i)
    {
      ...
      if (range->min() <= offsetInLine && offsetInLine < range->max())
      {
        ...
        {
          result.append(new ScopedTextRange(lineOffset + range->min(),
                                            lineOffset + range->max(),
                                            range->scope()));
        }
      }
    }
...
/// Returns a string-list will all scopes. This list is mainly used for
...
{
  qlog_info() << "dumpScopedLineAddresses(" << text
              << "): " << lineRangeList_.length();
  for (int i = 0, cnt = lineRangeList_.length(); i < cnt; ++i)
  {
    qlog_info() << "-" << i << ":"
                << QString::number((quintptr)lineRangeList_.at(i), 16);
  }
  qlog_info() << ".";
}
