...
private:
  ...
  bool scrollPastEnd_; ///< Should the last line of the document be  scrollable
                       ///< to the top of the window
  int showWhitespaceMode_;          ///< The current whitespace mode to make
  bool renderBidiContolCharacters_; ///< Renders dangers control characters as
                                    ///< red marks

  bool autocompleteAutoShow_; ///< Show autocomplete automatically, or only when
                              ///< manually triggered
  int autocompleteMinimalCharacters_; ///< How manu characters need to be
                                      ///< entered before autocomplete kicks in
};
