...
TextBufferChangeData::TextBufferChangeData(TextBuffer *buffer, int off, int len,
                                           const QChar *text, int textlen)
  : offset_(off)
  , length_(len)
  , newText_(text)
  ...
  , newLineOffsets_()
{
  ...
  for (int i = 0; i < newTextLength_; ++i)
  {
    if (newText_[i] == '\n')
    {
      newLineOffsets_.append(
          offset_ + i
          + 1); // +1 because it points to the start of the next line
    }
  }
}
...
/// Initializes the textbuffer change
/// @param buffer when buffer is 0 NO line calculation is done
TextBufferChangeData::TextBufferChangeData(LineOffsetVector *lineOffsets,
                                           int off, int len, const QChar *text,
                                           int textlen)
  : offset_(off)
  , length_(len)
  , newText_(text)
...
TextBufferChange::TextBufferChange(TextBuffer *buffer, int off, int len,
                                   const QChar *text, int textlen)
{
  d_ = new TextBufferChangeData(buffer, off, len, text, textlen);
}

TextBufferChange::TextBufferChange(LineOffsetVector *lineOffsets, int off,
                                   int len, const QChar *text, int textlen)
{
  d_ = new TextBufferChangeData(lineOffsets, off, len, text, textlen);
}

TextBufferChange::TextBufferChange(const TextBufferChange &other)
  : d_(other.d_)
{
}
...
/// A convenient method for directly filling the textbuffer with the given
/// content
...
/// @param line the line index this position is on. (Use this argument for
/// optimization if you already know this)
...
/// If the column exceed the number of column the caret is placed just before
/// the newline
int TextBuffer::offsetFromLineAndColumn(int line, int col)
{
  ...
  if (offset >= offsetNextLine && offset < length())
  {
    --offset;
  }
  return offset;
...
/// Returns the line at the given line position. This line INCLUDES the newline
...
QString TextBuffer::lineWithoutNewline(int line)
{
  ...
  if (line == lineCount() - 1)
  {
    removeNewlineCount = 0;
  }
  return textPart(off, offsetFromLine(line + 1) - off
                           - removeNewlineCount); // skip the return
}

/// Returns the length of the given line. Also counting the trailing newline
...
/// Returns the length of the given line. Without counting a trailing newline
/// character
/// @param line the line to retrieve the length for
/// @return the length of the given line
...
/// @param equals when setting to true if will search for the first given char.
/// When false it will stop when another char is found
int TextBuffer::findCharPos(int offset, int direction, const QString &chars,
                            bool equals)
{
  return findCharPosWithinRange(offset, direction, chars, equals, 0, length());
}

/// This method finds the find the first character position that equals the
/// given char
///
/// @param offset the offset to search from. A negative offset means the CURRENT
/// character isn't used
/// @param direction the direction to search. If the direction is multiple. the
/// nth item is returned
...
int TextBuffer::findCharPosWithinRange(int offset, int direction,
                                       const QString &chars, bool equals,
...
/// See documentation at findCharPosWithinRange.
/// This method searches a char position within the given rang (from the given
/// ofset)
int TextBuffer::findCharPosOrClamp(int offset, int direction,
                                   const QString &chars, bool equals)
{
  return findCharPosWithinRangeOrClamp(offset, direction, chars, equals, 0,
                                       length());
}
...
/// This method searches a char position within the given rang (from the given
/// ofset)
int TextBuffer::findCharPosWithinRangeOrClamp(int offset, int direction,
                                              const QString &chars, bool equals,
                                              int beginRange, int endRange)
{
  int pos = findCharPosWithinRange(offset, direction, chars, equals, beginRange,
                                   endRange);
  if (pos < 0)
  {
    if (direction < 0)
...
// This method converts the line offsets as a comma-seperated string (easy for
// debugging)
