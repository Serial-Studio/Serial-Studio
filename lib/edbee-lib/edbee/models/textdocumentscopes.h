...
/// on a certian location, usually more then one scope is available on a given
/// location
...
/// e.g.: (a | b) & c - d
///
/// would select the scope which is not matched by d, and matched by both c, and
/// a or b.
///
/// OFFICIAL BNF:
///   atom:         «string» | '*'
...
private:
  double calculateMatchScoreForSelector(TextScopeList *selector,
                                        const TextScopeList *scopeList);
...
private:
  MultiLineScopedTextRange
      *multiScopeRef_; ///< the reference to the multi-scoped textrange that
                       ///< defined this scope
};
...
private:
  ...
  bool independent_; ///< this boolean tells if the line contains a multi-lined
                     ///< scope start or end
...
public:
  ...
  static bool lessThan(MultiLineScopedTextRange *r1,
...
{
public:
  MultiLineScopedTextRangeSet(TextDocument *textDocument,
  ...
  virtual MultiLineScopedTextRange &
  addRange(int anchor, int caret, const QString &name, TextGrammarRule *rule);
...
private:
  TextDocumentScopes
      *textDocumentScopesRef_; ///< A reference to the text document scopes
  QList<MultiLineScopedTextRange *>
      scopedRangeList_; ///< A list of all scoped ranges
};
...
public:
  ...
  multiLineScopedRangesBetweenOffsets(int offsetBegin, int offsetEnd);
...
private:
  ...
  MultiLineScopedTextRange
      defaultScopedRange_; ///< The default scoped text range
  //    QHash<QString,TextScope*> scopeMap_;                 ///< A list of all
  //    defined/used scopes (pointers to these scopes are smaller then full
  //    strings)
  MultiLineScopedTextRangeSet
      scopedRanges_; ///< A list with all (multi-line) ranges
  GapVector<ScopedTextRangeList *>
      lineRangeList_; ///< A list of all line scopes

  /// This special variable is used to 'remember' to which offset the document
  /// has been scoped. This should speed up the syntax highlighting drasticly
  /// because the parsing only needs to happen to the end of the 'visible'
  /// document
  ///
  /// If you have an offset AFTER this offset the document has not been scoped
  /// yet. This needs to happen. For all 'open' multi-line scopes with an
  /// end-offset of (documentLength).
  ///
  /// The scopedToOffset_ should only mark the multi-line scopes. Single lines
  /// scopes do NOT affect other regions of the document
