...
TextUndoStack::TextUndoStack(TextDocument *doc, QObject *parent)
  ...
  , changeIndex_(0)
  , controllerIndexMap_()
  , persistedIndex_(0) // defaults to 0 because a blank/new document is
                       // perstisted (doesn't need to be saved)
  , undoGroupStack_()
...
void TextUndoStack::clear()
{
  ...
  changeIndex_ = 0;
  setPersistedIndex(
      -1); // clearing the undo-stack must result in a non-saved state
...
/// Warning when a change is MERGED, the original change is deleted!!! And the
/// original pointer is invalid!!!!
///
...
///        The return value != change if the change was merged  (previous stack
///        item is returned)
Change *TextUndoStack::giveChange(Change *change, int coalesceId)
{
  ...
  bool merge = lastCoalesceId && lastCoalesceIdStack_.top() == coalesceId
               && coalesceId != CoalesceId_None;

  setLastCoalesceIdAtCurrentLevel(coalesceId);
  if (coalesceId == CoalesceId_ForceMerge)
  {
    merge = true;
  }
  ...
  if (group)
  {
    ...
    // TODO: giveChange to a group can also merge a change. This can have
    // implications
    //   on the change. At the moment I simply return the group. I doubt this is
    //   correct for the moment it's the best solution for this problem
    return group;
...
/// This method returns the current redo change. This maybe the change for the
/// given context or the document change depending on what comes first
/// @param controller the controller this change is for
/// @return the new text change (0 if there aren't any redo items availabe for
/// the given context). When a controller is given the document change is ALSO
/// returned
...
/// @return the last change (with optional the controller context) 0 if no last
/// change
...
int TextUndoStack::currentIndexInDocChanges()
{
  ...
  {
    if (changeList_.at(i)->isDocumentChange())
    {
      ++index;
    }
  }
...
bool TextUndoStack::isPersisted()
{
  ...
  if (persistedIndex_ == curIdx)
  {
    return true;
  }
  if (persistedIndex_ < 0)
  {
    return false;
  }
...
QString TextUndoStack::dumpStack()
{
  ...
  for (int i = changeList_.size(); i >= 0; --i)
  {
    ...
    for (itr = controllerIndexMap_.begin(); itr != controllerIndexMap_.end();
         ++itr)
    {
      if (itr.value() == i)
      {
        pointers.append(QStringLiteral("<=(%1) ").arg(
            QString::number((quintptr)itr.key(), 16)));
      }
    }
    if (changeIndex_ == i)
    {
...
/// @return the next change index. Or changeList_.size() if there's no next
/// change
int TextUndoStack::findRedoIndex(int index, TextEditorController *controller)
{
  if (index < changeList_.size())
  {
    ...
    for (; index < size; ++index)
    {
      ...
      if (context == 0 || context == controller)
      {
        return index;
      }
    }
...
void TextUndoStack::clearRedo(TextEditorController *controller)
{
  ...
  if (controller)
  {
    ...
    {
      Q_ASSERT(false
               && "The current controller isn't registered with the "
                  "undostack!"); // warning view isn't registered!
    }
...
/// This method undos the given controller change. This method does NOT undo
/// document changes
...
{
  Q_ASSERT(controller);
  int changeIdx = controllerIndexMap_.value(controller)
                  - 1; // -1, because the index is directly AFTER the item
...
{
  // first make sure ALL controller-changes are back to the document
  // change-level
  QMap<TextEditorController *, int>::iterator itr;
  for (itr = controllerIndexMap_.begin(); itr != controllerIndexMap_.end();
       ++itr)
  {
    ...
    while (undoControllerChange(controller))
    {
    }; // undo all controller operations
  }
  ...
  if (change)
  {
    ...
    {
      TextEditorController *controller = itr.key();
      controllerIndexMap_[controller]
          = findUndoIndex(changeIndex_ - 1, controller)
            + 1; // +1 because the pointer points AFTER the found index
    }

    // and finally move the document pointer
    setChangeIndex(findUndoIndex(changeIndex_ - 1)
                   + 1); // +1 because the pointer points AFTER the found index
...
void TextUndoStack::redoDocumentChange()
{
  ...
  // first move all controller redo-operation to the given redo found redo
  // location
  QMap<TextEditorController *, int>::iterator itr;
  for (itr = controllerIndexMap_.begin(); itr != controllerIndexMap_.end();
       ++itr)
  {
    TextEditorController *controller = itr.key();
    while (redoControllerChange(controller))
    {
    };                       // undo all controller operations
