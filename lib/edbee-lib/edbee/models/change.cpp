...
/// Gives the change and merges it if possible. This method should return false
/// if the change couldn't be merged. When the method returns true the ownership
/// of the given textchange is transfered to this class.
/// @param document the document this change is for
/// @param textChange the textchange
/// @return true if the merge succeeded. The textChange ownership is only
/// tranfered if true if returend
...
/// The default behaviour is that every textchange requires persistence. It is
/// also possible to have certain changes that do not require persitence but
/// should be placed on the undo stack
...
/// This method returns true if this change is a group change. When an object is
/// group change it should be inherited by TextChangeGroup
bool Change::isGroup()
{
  return false;
}
...
/// A controller specific textcommand. Warning you should NOT modify the
/// textdocument!
...
/// This method is called it the group is discardable. A discardable group will
/// be optimized away if the group is empty, or if there's a single item in the
/// group. A none-discardable group will always remain
...
/// This method flattens the undo-group by expanding all subgroups to local
/// groups
void ChangeGroup::flatten()
{
  for (int idx = 0; idx < changeList_.size(); ++idx)
  {
    ...
    if (group)
    {
      ...
      {
        ChangeGroup *subGroup
            = dynamic_cast<ChangeGroup *>(group->changeList_[i]);
        if (subGroup)
        {
          subGroup->flatten(); // flatten the group
          delete subGroup;
          if (i >= group->size())
            break; // just make sure empty groups go right (in theory empty
                   // groups aren't possible)
        }
...
TextEditorController *ChangeGroup::controllerContext()
{
  ...
  for (int i = size() - 1; i >= 0; --i)
  {
    ...
    if (commandContext == nullptr)
      return nullptr; /// 0 is always 0!
    if (commandContext && context && commandContext != context)
    {
      return nullptr;
    }
