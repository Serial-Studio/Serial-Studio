...
/// When you call beginChanges this method will make sure the config surpresses
/// the signals and only a signal is fired after the last endChange. You can
/// nest multiple beginChanges, only the last endChanges fires a signal
...
/// Sets the characters that are considered whitespace. Excluding the newline
/// character
/// @param value the list of characters that are considered whitespace
/// characters
...
/// closing of an undo group. (This groups the entered characters into 1 undo
/// operation)
...
/// (The theme name is the name without the file-extension of the file in the
/// theme directory)
...
/// @param name the name of the active theme (this is the filename of the theme
...
/// When enabled the last line of the document is scrollable to the top of the
/// window
...
/// @param mode the whitespace mode to set. Can be one of the enumeration
/// values. (Or another integer if you build your own renderer)
...
/// @param str the whitespace mode "show" or "hide" for now. (When invalid hide
/// is used)
...
/// For showing possible security related characters:
/// https://www.trojansource.codes Default is Enabled!
void TextEditorConfig::setRenderBidiContolCharacters(bool enabled)
{
  renderBidiContolCharacters_ = enabled;
}

/// Sets whether autocomplete comes up automatically, or only manually(manual
/// trigger isn't implemented yet)
...
/// Show autocomplete automatically, or only manually(manual isn't implemented
/// yet)
...
/// This internal method is used to notify the listener that a change has
/// happend Thi smethod only emits a signal if there's no config group change
/// busy
