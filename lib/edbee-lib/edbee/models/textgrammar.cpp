...
TextGrammarRule *TextGrammarRule::createMainRule(TextGrammar *grammar,
...
TextGrammarRule *TextGrammarRule::createIncludeRule(TextGrammar *grammar,
...
TextGrammarRule *TextGrammarRule::createSingleLineRegExp(
    TextGrammar *grammar, const QString &scopeName, const QString &regExp)
{
...
TextGrammarRule *TextGrammarRule::createMultiLineRegExp(
    TextGrammar *grammar, const QString &scopeName,
    const QString &contentScopeName, const QString &beginRegExp,
...
QString TextGrammarRule::toString(bool includePatterns)
{
  ...
  switch (instruction_)
  {
    ...
    case RuleList:
      r.append("RuleList");
      break;
    case SingleLineRegExp:
      r.append("SingleLineRegExp");
      break;
    case MultiLineRegExp:
      r.append("MultiLineRegExp");
      break;
    case IncludeCall:
      r.append("IncludeCall");
      break;
    case Parser:
      r.append("Parser");
      break;
  ...
  if (includePatterns && matchRegExp_)
  {
    r.append(", begin: ").append(matchRegExp_->pattern());
  }
  if (includePatterns && !endRegExpString_.isEmpty())
  {
    r.append(", end: ").append(endRegExpString_);
  }
...
RegExp *TextGrammarRule::createRegExp(const QString &regexp)
{
  RegExp *result = new RegExp(
      regexp,
      RegExp::EngineOniguruma); //, Qt::CaseSensitive, QRegExp::RegExp2 );
  if (!result->isValid())
  {
...
TextGrammarRule *TextGrammar::findFromRepos(const QString &name,
                                            TextGrammarRule *defValue)
{
  ...
  : defaultGrammarRef_(0)
{
  ...
  defaultGrammarRef_->giveMainRule(
      TextGrammarRule::createMainRule(defaultGrammarRef_, "text.plain"));
...
TextGrammar *TextGrammarManager::readGrammarFile(const QString &file)
{
  ...
  else
  {
    ...
    lastErrorMessage_ = QObject::tr("Error reading file %1:%2")
...
void TextGrammarManager::giveGrammar(TextGrammar *grammar)
{
  ...
  if (grammarMap_.contains(name))
  {
    ...
    // when the old grammar was the default, replace the default (feels pretty
    // dirty)
...
static bool grammarsDisplayNameSorterLessThen(const TextGrammar *g1,
                                              const TextGrammar *g2)
{
  return g1->displayName().toLower() < g2->displayName().toLower();
}

/// This utility function returns all grammars sorted on displayname
/// Warning, this method sorts the grammars so calling this method multiple
/// times is not what you want to do
...
TextGrammar *
TextGrammarManager::detectGrammarWithFilename(const QString &fileName)
{
  foreach (TextGrammar *grammar, grammarMap_.values())
  {
    foreach (QString ext, grammar->fileExtensions())
    {
      if (fileName.endsWith(QStringLiteral(".%1").arg(ext)))
        return grammar;
    }
  }
