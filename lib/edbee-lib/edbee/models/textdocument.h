...
/// - A textbuffer, which holds the character data. Currently there's only a
/// CharTextBuffer, a gap-vectored buffer.
/// - An undostack, a stack which holds the undo-operations of the editor
/// - the textdocument scopes, these are the language-dependent scopes found in
...
public:
  ...
  /// Warning you should NEVER directly modify the textbuffer unless you're
  ...
  /// This method should emit a grammarChanged signal (if the grammar is
  /// changed)
...
private:
  TextDocumentFilter
      *documentFilter_; ///< The document filter if the filter is owned
  TextDocumentFilter
      *documentFilterRef_; ///< The reference to the document filter.

  TextLineDataManager
      *textLineDataManager_; ///< A class for managing text line data items
};
