...
public:
  ...
  int moveWhileChar(TextDocument *doc, int pos, int amount,
                    const QString &chars);
  int moveUntilChar(TextDocument *doc, int pos, int amount,
                    const QString &chars);
  ...
  void moveAnchortWhileChar(TextDocument *doc, int amount,
                            const QString &chars);
  void moveAnchorUntilChar(TextDocument *doc, int amount, const QString &chars);
  void moveCaretByCharGroup(TextDocument *doc, int amount,
                            const QString &whitespace,
                            const QStringList &characterGroups);
  void moveCaretToLineBoundary(TextDocument *doc, int amount,
                               const QString &whitespace);
  ...
  void expandToWord(TextDocument *doc, const QString &whitespace,
                    const QStringList &characterGroups);
...
/// when changing is 0. This way it possible to add/update muliple rages without
/// the direct performance hit of sorting and merging.
...
public:
  ...
  bool rangesBetweenOffsets(int offsetBegin, int offsetEnd, int &firstIndex,
                            int &lastIndex);
  bool rangesBetweenOffsetsExlusiveEnd(int offsetBegin, int offsetEnd,
  ...
  void expandToWords(const QString &whitespace,
                     const QStringList &characterGroups);
  void selectWordAt(int offset, const QString &whitespace,
                    const QStringList &characterGroups);
  void toggleWordSelectionAt(int offset, const QString &whitespace,
                             const QStringList &characterGroups);

  // movement
  void moveCarets(int amount);
  void moveCaretsOrDeselect(int amount);
  void moveCaretsByCharGroup(int amount, const QString &whitespace,
                             const QStringList &charGroups);
  void moveCaretsToLineBoundary(int direction, const QString &whitespace);
  //    void moveCaretsByLine( int amount );  //< Impossible to do without view
  //    when having a flexible font, guess that's why it wasn't implemented
...
protected:
  ...
  int changing_; ///< A (integer) boolean for handling changes between
                 ///< beginChagnes and endChanges
};

//======================================================================

/// The basic textrange class. A simple class of textrange with a simple vector
/// implementation
...
private:
  QVector<TextRange>
      selectionRanges_; ///< A list of selection ranges. After endChanges this
                        ///< array is sorted and non-overlapping!
};

//======================================================================

/// A smart QObject implemenation of a TextRangeSet which listens to changes
/// in the document. When a change happens it's changes the spatial of the
/// ranges
///
/// The stickymode is used to change the behavior of the changes the the
/// textChange event. To put it simple, you should enable stickymode if this
/// selection is the one you are using to modify the document
///
/// The delete mode is used to tell the rangeset if 'deleted' ranges need to be
/// deleted or simply need to be moved
class EDBEE_EXPORT DynamicTextRangeSet : public QObject, public TextRangeSet
{
  Q_OBJECT

public:
  DynamicTextRangeSet(TextDocument *doc, bool stickyMode = false,
                      bool deleteMode = false, QObject *parent = 0);
  DynamicTextRangeSet(const TextRangeSet &sel, bool stickyMode = false,
                      bool deleteMode = false, QObject *parent = 0);
  DynamicTextRangeSet(const TextRangeSet *sel, bool stickyMode = false,
                      bool deleteMode = false, QObject *parent = 0);
...
private:
  bool stickyMode_; ///< Sticky mode means if this rangeset is the current
                    ///< selection (This requires a different approach)
  bool deleteMode_; ///< When delete mode is enabled ranges are deleted. If it's
                    ///< false ranges are moved to the left
};
