...
TextAutoCompleteItem::TextAutoCompleteItem(const QString &label, const int kind,
...
*/
int TextAutoCompleteItem::matchLabelScore(TextDocument *doc, const TextRange &,
                                          const QString &word)
{
...
QList<TextAutoCompleteItem *>
StringTextAutoCompleteProvider::findAutoCompleteItemsForRange(
    TextDocument *document, const TextRange &range, const QString &word)
{
  ...
  foreach (TextAutoCompleteItem *item, itemList_)
  {
    ...
    if (match && match == 1
        && item->kind() == edbee::TextAutoCompleteKind::Keyword)
    {
      items.clear();
      return items.values();
    }
...
void StringTextAutoCompleteProvider::add(const QString &label, const int kind,
                                         const QString &detail,
                                         const QString &documentation)
{
  itemList_.push_back(new TextAutoCompleteItem(
      label, kind, detail == "" && kind == 3 ? label + "()" : detail,
      documentation));
}
...
TextAutoCompleteProviderList::TextAutoCompleteProviderList(
    TextAutoCompleteProvider *parentProvider)
...
QList<TextAutoCompleteItem *>
TextAutoCompleteProviderList::findAutoCompleteItemsForRange(
    TextDocument *document, const TextRange &range, const QString &word)
{
  ...
  {
    result.append(
        provider->findAutoCompleteItemsForRange(document, range, word));
  }
  if (parentProviderRef_)
  {
    result.append(parentProviderRef_->findAutoCompleteItemsForRange(
        document, range, word));
  }
  return result;
}
...
/// adds a provider
void TextAutoCompleteProviderList::giveProvider(
    TextAutoCompleteProvider *provider)
{
  providerList_.push_back(provider);
}

/// set the parent provider
void TextAutoCompleteProviderList::setParentProvider(
    TextAutoCompleteProvider *provider)
{
...
QList<TextAutoCompleteItem *>
ScopeBasedTextAutoCompleteProvider::findAutoCompleteItemsForRange(TextDocument
...
void
TextAutoCompleteManager::giveProvider(TextAutoCompleteManager::~TextAutoCompleteProvider
*provider)
{
