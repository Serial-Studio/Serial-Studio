  ...
  , readonly_(false)
{
  ...
  //    scrollAreaRef_->setLeftWidget( new QLabel("Left",this)
  //    );//marginCompRef_ ); scrollAreaRef_->setTopWidget( new
  //    QLabel("Top",this)); scrollAreaRef_->setRightWidget( new
  //    QLabel("Right",this)); scrollAreaRef_->setBottomWidget( new
  //    QLabel("Bottom",this));
  ...
  autoCompleteCompRef_ = new TextEditorAutoCompleteComponent(
      controller_, editCompRef_, marginCompRef_);

  marginCompRef_->init();
  connectHorizontalScrollBar();
  connectVerticalScrollBar();
  connect(this, SIGNAL(horizontalScrollBarChanged(QScrollBar *)),
          SLOT(connectHorizontalScrollBar()));
  connect(this, SIGNAL(verticalScrollBarChanged(QScrollBar *)),
          SLOT(connectVerticalScrollBar()));
  connect(editCompRef_, SIGNAL(textKeyPressed()), autoCompleteCompRef_,
          SLOT(textKeyPressed()));
  connect(controller_, SIGNAL(backspacePressed()), autoCompleteCompRef_,
          SLOT(backspacePressed()));

  setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);

  editCompRef_->installEventFilter(
      this); // receive events for the ability to emit focus events
}
...
{
  scrollAreaRef_->ensureVisible(xPosIn, yPosIn, autoScrollMargin_,
                                autoScrollMargin_);
}
...
/// Use this method to change a scrollbar. By using this method listeners of the
/// scrollbar well be informed the scrollbar has been changed
...
/// Use this method to change the horizontal scrollbar.  By using this method
/// listeners of the scrollbar well be informed the scrollbar has been changed
/// @param scrollBar the scrollbar to use
...
{
  QRect rect(horizontalScrollBar()->value(), this->verticalScrollBar()->value(),
             scrollAreaRef_->viewport()->width(), scrollAreaRef_->height());
