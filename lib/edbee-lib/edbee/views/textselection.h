...
///
/// For instance moving line/up and line/down isn an operation that isn't
/// possible on the document side. Because we support variable font widts.. Also
/// 'remembering' the caret screen-x and -y positions isn't relevant for a
/// docuemnt. That is stuff that needs to be placed in the view side
...
public:
  ...
  static void moveCaretsByLine(TextEditorController *controller,
                               TextRangeSet *rangeSet, int amount);
  static void moveCaretsByPage(TextEditorController *controller,
                               TextRangeSet *rangeSet, int amount);
  static void addRangesByLine(TextEditorController *controller,
                              TextRangeSet *rangeSet, int amount);
