...
/// When another rangeSet is given this code is execute on that rangeset! (FIXME
...
void TextSelection::moveCaretsByLine(TextEditorController *controller,
                                     TextRangeSet *rangeSet, int amount)
{
  ...
  for (int rangeIdx = rangeSet->rangeCount() - 1; rangeIdx >= 0; --rangeIdx)
  {
    ...
    range.setCaretBounded(
        doc, qMin(offset + col, offsetNextLine - newLinesToRemove));

    //        cache->caretMovedFromOldOffsetToNewOffset( caret, range.caret() );
    //        // when having multiple caret this doesn't work !
...
void TextSelection::moveCaretsByPage(TextEditorController *controller,
                                     TextRangeSet *rangeSet, int amount)
{
  TextRenderer *renderer = controller->textRenderer();
  TextDocument *doc = controller->textDocument();
...
void TextSelection::addRangesByLine(TextEditorController *controller,
                                    TextRangeSet *rangeSet, int amount)
{
  TextDocument *doc = controller->textDocument();
  TextRenderer *renderer = controller->textRenderer();
