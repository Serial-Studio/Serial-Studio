...
TextThemeRule::TextThemeRule(const QString &name, const QString &selector,
                             QColor foreground, QColor background, bool bold,
...
void TextThemeRule::fillFormat(QTextCharFormat *format)
{
  ...
  {
    format->setForeground(foregroundColor_);
  }
  if (backgroundColor_.isValid())
  {
    format->setBackground(backgroundColor_);
  }
  if (bold_)
  {
...

{
  ...
  //    )); giveThemeRule( new TextThemeRule("String","string",
  //    QColor("#E6DB74") ));
}
...
void TextTheme::fillFormatForTextScopeList(const TextScopeList *scopeList,
  ...
  , themeRef_(0)
{
  connect(
      controller,
      SIGNAL(textDocumentChanged(edbee::TextDocument *, edbee::TextDocument *)),
      SLOT(textDocumentChanged(edbee::TextDocument *, edbee::TextDocument *)));
  connect(Edbee::instance()->themeManager(),
          SIGNAL(themePointerChanged(QString, TextTheme *, TextTheme *)),
          SLOT(themePointerChanged(QString, TextTheme *, TextTheme *)));
...
TextThemeStyler::getLineFormatRanges(int lineIdx)
{
  ...
  if (scopedRanges == 0 || scopedRanges->size() == 0)
  {
  ...
  for (int i = 1, cnt = scopedRanges->size(); i < cnt; ++i)
  {
    ...
    while (activeRanges.size() > 1)
    {
      ...
      // when the 'min' is behind the end of the textrange on the stack we need
      // to pop the stack
      if (activeRangeMax <= min)
      {
        appendFormatRange(formatRangeList, lastOffset, activeRangeMax - 1,
                          activeRanges);
        activeRanges.pop();
...
/// It returns a NullString if the theme is specified manually (without the
/// thememanager)
...
QTextCharFormat
TextThemeStyler::getTextScopeFormat(QVector<ScopedTextRange *> &activeRanges)
{
...
void TextThemeStyler::appendFormatRange(
    QVector<QTextLayout::FormatRange> &rangeList, int start, int end,
    QVector<ScopedTextRange *> &activeRanges)
{
...
void TextThemeStyler::textDocumentChanged(edbee::TextDocument *oldDocument,
...
void TextThemeStyler::themePointerChanged(const QString &name,
                                          TextTheme *oldTheme,
                                          TextTheme *newTheme)
{
  if (name == themeName_)
  ...
  else
  {
    ...
    {
      Q_ASSERT(false
               && "The old theme is deleted but it's not the same theme name. "
...
/// This method loads all theme names from the given theme path (*.tmTheme
/// files)
/// @param the new themePath. if the themepath is blank, the themes of the
/// current path are reloaded
void TextThemeManager::listAllThemes(const QString &themePath)
{
  ...
  if (!themePath_.isEmpty())
  {
    ...
    QFileInfoList fileInfoList
        = dir.entryInfoList(QStringList("*.tmTheme"), QDir::Files, QDir::Name);
...
/// @param name the name of the theme (if the name isn't given the basenaem of
/// the fileName is used (excluding the .tmTheme extension)
/// @return the loaded theme or 0 if the theme couldn' be loaded
TextTheme *TextThemeManager::readThemeFile(const QString &fileName,
                                           const QString &nameIn)
{
  ...
  if (file.exists() && file.open(QIODevice::ReadOnly))
  {
    ...
    {
      lastErrorMessage_ = QObject::tr("Error parsing theme %1:%2")
                              .arg(file.fileName())
                              .arg(parser.lastErrorMessage());
    }
    file.close();
    return theme;
  }
  ...
  {
    lastErrorMessage_
        = QObject::tr("Error theme not found %1.").arg(file.fileName());
    return 0;
  }
}
...
TextTheme *TextThemeManager::theme(const QString &name)
{
  ...
  {
    QString filename
        = QStringLiteral("%1/%2.tmTheme").arg(themePath_).arg(name);
