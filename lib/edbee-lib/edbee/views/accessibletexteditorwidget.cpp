...
/// This works for Windows Reader and Mac OS X Speaker, but doesn't seem to work
/// for NVDA The TexteditorComponent is the component that has the true focus
#define VIA_EDITOR_COMPONENT

/// QT Acessibility has an issue with reporting blank lines between elements
/// lines. defining 'WINDOWS_EMPTY_LINE_READING_ERROR_FIX' adds a \r before a
/// newline. Which is a workaround for this issue. (It does some offset-length
/// and string magic when this is enabled)
...
#  endif // WINDOWS_LAST_LINE_ERROR_FIX - First use
  // qDebug() << "VTEXT_PART: " << offset << ", " << length << " :: docLength: "
  // << docLength << ", endOffset: " << endOffset;
...
AccessibleTextEditorWidget::factory(const QString &className, QObject *object)
{
  // edbee::TextMarginComponent, edbee::TextEditorScrollArea,
  // edbee::TextEditorComponent
#ifdef VIA_EDITOR_COMPONENT
  if (className == QLatin1String("edbee::TextEditorComponent") && object
      && object->isWidgetType())
  {
    return new AccessibleTextEditorWidget(
        static_cast<edbee::TextEditorComponent *>(object)
            ->controller()
            ->widget());
  }
#else
  if (className == QLatin1String("edbee::TextEditorWidget") && object
      && object->isWidgetType())
  {
    return new AccessibleTextEditorWidget(
        static_cast<edbee::TextEditorWidget *>(object));
  }
...
QWidget *
AccessibleTextEditorWidget::eventWidgetForTextEditor(TextEditorWidget *widget)
{
#ifdef VIA_EDITOR_COMPONENT
...
void AccessibleTextEditorWidget::notifyTextSelectionEvent(
    TextEditorWidget *widget, TextSelection *selection)
{
  ...
  for (int i = 0, cnt = selection->rangeCount(); i < cnt; ++i)
  {
    ...
    QAccessibleTextSelectionEvent ev(eventWidget, D2V(widget, range.min()),
                                     D2V(widget, range.max()));
    ev.setCursorPosition(D2V(widget, range.caret()));

    //        qDebug() << " !!updateAccessibility:
    //        QAccessibleTextSelectionEvent: " << range.min()<< ", " <<
    //        range.max() << ", " << range.caret();
    ...
    {
      const QAccessibleTextInterface *ti
          = QAccessible::queryAccessibleInterface(eventWidget)->textInterface();
...
void AccessibleTextEditorWidget::notifyTextChangeEvent(TextEditorWidget *widget,
                                                       ...
                                                       QString oldDocText)
{
  ...
  QAccessibleTextUpdateEvent ev(eventWidget, D2V(widget, change->offset()),
                                oldText, newText);
  ...
  //    qDebug() << "!! updateAccessibility: QAccessibleTextUpdateEvent: " <<
  //    change->offset()<< ", oldText: " << oldText << ", newText: " << newText;
...
/// and endOffset. If there is no selection both startOffset and endOffset are
/// nullptr.
///
...
void AccessibleTextEditorWidget::selection(int selectionIndex, int *startOffset,
                                           int *endOffset) const
{
  ...
  *endOffset = D2V(textWidget(), range.max());
  // qDebug() << " selection >> " << selectionIndex << ", " << range.min() << "
  // =>" << *startOffset << ", " << range.max() << " => " << *endOffset;
}
...
int AccessibleTextEditorWidget::selectionCount() const
{
  if (!textSelection()->hasSelection())
    return 0; // no selection (only caret)
  return textSelection()->rangeCount();
}

/// Select the text from startOffset to endOffset. The startOffset is the first
/// character that will be selected. The endOffset is the first character that
...
void AccessibleTextEditorWidget::addSelection(int startOffset, int endOffset)
{
  ...
  selection.addRange(V2D(textWidget(), startOffset),
...
void AccessibleTextEditorWidget::setSelection(int selectionIndex,
                                              int startOffset, int endOffset)
{
  TextSelection selection = *textSelection();
  selection.setRange(V2D(textWidget(), startOffset),
                     V2D(textWidget(), endOffset), selectionIndex);
  controller()->changeAndGiveTextSelection(&selection);
}
...
{
  // qDebug() << "AccessibleTextEditorWidget::setCursorPosition: " << position
  // << " => <" << V2D(textWidget(), position);
...
/// Returns the text from startOffset to endOffset. The startOffset is the first
/// character that will be returned. The endOffset is the first character that
...
QString AccessibleTextEditorWidget::text(int vStartOffset, int vEndOffset) const
{
  ...
  // qDebug() << "text: " << VTEXT_PART(textWidget()->textDocument(),
  // startOffset, endOffset - startOffset); qDebug() << "      - vStartOffset "
  // << vStartOffset << ", vEndOffset " << vEndOffset ; qDebug() << "      -
  // startOffset " << startOffset << ", endOffset: " << endOffset << ", len: "
  // << endOffset-startOffset << " (docLength: " <<
  // textWidget()->textDocument()->length() <<")";
  return VTEXT_PART(textWidget()->textDocument(), startOffset,
...
/// Returns the position and size of the character at position offset in screen
/// coordinates.
QRect AccessibleTextEditorWidget::characterRect(int vOffset) const
{
  ...
  // qDebug() << " characterRect >>" << vOffset << " => " << offset << ": " <<
  // pointScreen;
  return QRect(pointScreen.x(), pointScreen.y(), renderer()->emWidth(),
               renderer()->lineHeight());
}
...
/// In addition the range of the attributes is returned in startOffset and
/// endOffset.
QString AccessibleTextEditorWidget::attributes(int offset, int *startOffset,
                                               int *endOffset) const
{
  Q_UNUSED(offset)
  Q_UNUSED(startOffset)
  Q_UNUSED(endOffset)
  // qDebug() << " attributes >>" << offset << ", " << *startOffset << ", " <<
  // *endOffset;
  return QString();
}

/// Returns the text item of type boundaryType that is right after offset offset
/// and sets startOffset and endOffset values to the start and end positions of
...
/// processor or text editor should provide their own efficient implementations.
...
/// convention an offset of -2 means that this function should use the cursor
/// position as offset. Thus an offset of -2 must be converted to the cursor
/// position before calling this function. An offset of -1 is used for the text
/// length and custom implementations of this function have to return the result
/// as if the length was passed in as offset.
QString AccessibleTextEditorWidget::textAfterOffset(
    int vOffset, QAccessible::TextBoundaryType boundaryType, int *startOffset,
    int *endOffset) const
{
  if (boundaryType == QAccessible::LineBoundary && vOffset >= 0)
  {
    ...
    *endOffset = *startOffset + str.length(); // D2V(textWidget(), end);
    ...
    //        qDebug() << "textAfterOffset: vOffset: " << vOffset << " => " <<
    //        offset
  ...
  QString str = QAccessibleTextInterface::textAfterOffset(
      vOffset, boundaryType, startOffset, endOffset);
  //    qDebug() << "?? textAfterOffset: vOffset: " << vOffset << ",
  //    boundaryType: " << boundaryType << " => " << str << " (" << *startOffset
  //    << ", " << *endOffset <<")";
  return str;
}
...
/// item; returns an empty string if there is no such an item. Sets startOffset
/// and endOffset values to -1 on error.
///
/// This default implementation is provided for small text edits. A word
/// processor or text editor should provide their own efficient implementations.
/// This function makes no distinction between paragraphs and lines.
...
/// length and custom implementations of this function have to return the result
/// as if the length was passed in as offset.
QString AccessibleTextEditorWidget::textAtOffset(
    int vOffset, QAccessible::TextBoundaryType boundaryType, int *startOffset,
    int *endOffset) const
{
  ...
  if (boundaryType == QAccessible::LineBoundary && vOffset >= 0)
  {
    ...
    *endOffset = (*startOffset) + str.length(); // D2V(textWidget(), end);
    ...
    //        qDebug() << "textAtOffset: vOffset: " << vOffset << " => " <<
    //        offset
    //                 << ", startOffset: " << *startOffset << " (start: " <<
    //                 start << ")"
    //                 << ", endOffset: " << *endOffset << " (end: " << end <<
  ...
  QString str = QAccessibleTextInterface::textAtOffset(vOffset, boundaryType,
                                                       startOffset, endOffset);
  //    qDebug() << "?? textAtOffset: offset: " << vOffset << ", boundaryType: "
  //    << boundaryType << " => " << str << " (" << *startOffset << ", " <<
  //    *endOffset <<")";
  return str;
}
...
///
/// This default implementation is provided for small text edits. A word
/// processor or text editor should provide their
///  own efficient implementations. This function makes no distinction between
...
/// length and custom implementations of this function have to return the result
/// as if the length was passed in as offset.
QString AccessibleTextEditorWidget::textBeforeOffset(
    int offset, QAccessible::TextBoundaryType boundaryType, int *startOffset,
    int *endOffset) const
{
  // qDebug() << "?? textBeforeOffset: offset: " << offset << ", boundaryType: "
  // << boundaryType;
...
/// It returns the location of the textWidget (even when the TextComponent has
/// got focus)
QRect AccessibleTextEditorWidget::rect() const
{
  QRect widgetRect = textWidget()->rect();
  QRect focusRect(textWidget()->mapToGlobal(widgetRect.topLeft()),
                  widgetRect.size());
  return focusRect;
}
