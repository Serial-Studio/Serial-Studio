...
class EDBEE_EXPORT TextEditorAutoCompleteComponent : public QWidget
{
  Q_OBJECT
public:
  explicit TextEditorAutoCompleteComponent(TextEditorController *controller,
...
protected:
  ...
  void hideInfoTip();
  bool fillAutoCompleteList(TextDocument *document, const TextRange &range,
                            const QString &word);
...
private:
  ...
  TextEditorComponent
      *editorComponentRef_; ///< Reference to the editor component
  TextMarginComponent
      *marginComponentRef_; ///< Reference to the editor's margin
  bool eventBeingFiltered_; ///< Prevent endless double filter when forwarding
                            ///< event to list item
