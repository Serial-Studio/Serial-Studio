...
void TextMarginComponentDelegate::renderBefore(QPainter *painter, int startLine,
                                               int endLine, int width)
{
  Q_UNUSED(painter)
  Q_UNUSED(startLine)
  Q_UNUSED(endLine)
...
void TextMarginComponentDelegate::renderAfter(QPainter *painter, int startLine,
                                              int endLine, int width)
{
  Q_UNUSED(painter)
  Q_UNUSED(startLine)
...
/// To use it you MUST manually call setMouseTracking() on the
/// TextMarginComponent
  ...
  if (event->buttons() & Qt::LeftButton)
  {
    if (line >= 0)
    {
      TextEditorController *controller
          = marginComponent()->editorWidget()->controller();
      if (line < startLine_)
      {
...
TextMarginComponent::TextMarginComponent(TextEditorWidget *editor,
                                         QWidget *parent)
  : QWidget(parent)
  , top_(0)
  , width_(-1)
...
void TextMarginComponent::init()
{
  ...
  updateMarginFont();
  connect(editorRef_->textRenderer(), SIGNAL(themeChanged(TextTheme *)),
          SLOT(updateFont()));
...
void TextMarginComponent::updateMarginFont()
{
  ...
  if (renderer()->textWidget())
  {
    ...
    if (font.pointSizeF() > 0)
      marginFont_->setPointSizeF(font.pointSizeF());
    if (font.pixelSize() > 0)
      marginFont_->setPixelSize(font.pixelSize());
  }
  else
  {
...
int TextMarginComponent::widthHint() const
{
  ...
  if (lastLineCount_ != lineCount)
  {
    ...
    width_ = qMax(renderer()->nrWidth(), 10) * (decimals)
             + LineNumberRightPadding + MarginPaddingRight;
...
void TextMarginComponent::setDelegate(TextMarginComponentDelegate *delegate)
{
  ...
  if (delegate == nullptr)
  {
    delegate = new TextMarginComponentDelegate();
  }
...
void TextMarginComponent::paintEvent(QPaintEvent *event)
{
  ...
  // qlog_info()<< "** top:"<<top_<<", total:"<<totalRect<<", paintRect:
  // "<<paintRect<<" => rect: " << rect;
...
void TextMarginComponent::renderCaretMarkers(QPainter *painter, int startLine,
                                             int endLine, int width)
{
  TextDocument *doc = renderer()->textDocument();
...
/// @param endLine the last line to render
/// @param width the width for rendering
void TextMarginComponent::renderLineNumber(QPainter *painter, int startLine,
                                           int endLine, int width)
{
  ...
  int lineHeight = renderer()->lineHeight();
  int textWidth = width - LineNumberRightPadding - MarginPaddingRight
                  - delegate()->widthBeforeLineNumber();
  ...
  for (int line = startLine; line <= endLine; ++line)
  {
    ...
    if (renderer()->textSelection()->rangesAtLineExclusiveEnd(line, firstIndex,
                                                              lastIndex))
    {
    ...
    painter->drawText(delegate()->widthBeforeLineNumber(), y + 2, textWidth,
                      lineHeight, Qt::AlignRight, delegate()->lineText(line));
  }
}
...
/// forward the mouse wheel event to the scrollbar so you can use the mouse
/// wheel on this component
void TextMarginComponent::wheelEvent(QWheelEvent *event)
{
  //    if( event->orientation() == Qt::Vertical) {
  if (event->angleDelta().y() != 0)
  {
    QApplication::sendEvent(editorRef_->textScrollArea()->verticalScrollBar(),
                            event);
  }
}
...
{
  connect(editorRef_->verticalScrollBar(), SIGNAL(valueChanged(int)), this,
          SLOT(topChanged(int)));
}
