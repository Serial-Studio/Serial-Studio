  ...
  , shadowGradient_(0)
{
  ...
  shadowGradient_->setColorAt(0, QColor(0x00, 0x00, 0x00, 0x00));
...
void TextEditorRenderer::render(QPainter *painter)
{
  ...
  //    renderShade( painter, *renderer()->clipRect() );        /// TODO, deze
  //    renderShade moet misschien in de viewport render-code gebreuren
}
...
void TextEditorRenderer::renderLineSelection(QPainter *painter, int line)
{
  ...
  int firstRangeIdx = 0;
  int lastRangeIdx = 0;
  /// TODO: iprove ranges at line by calling rangesForOffsets first for only the
  /// visible offsets!
  if (sel->rangesAtLine(line, firstRangeIdx, lastRangeIdx))
  {
    ...
    for (int rangeIdx = firstRangeIdx; rangeIdx <= lastRangeIdx; ++rangeIdx)
    {
      ...
      if (range.length() > 0 && endColumn + 1 >= lastLineColumn)
        endX += 3;

      painter->fillRect(
          startX, line * lineHeight + rect.top(), endX - startX, rect.height(),
          themeRef_->selectionColor()); // QColor::fromRgb(0xDD, 0x88, 0xEE) );
    }
  }
  // PROF_END
}
...
void TextEditorRenderer::renderLineBorderedRanges(QPainter *painter, int line)
{
  ...
  int firstRangeIdx = 0;
  int lastRangeIdx = 0;
  /// TODO: improve ranges at line by calling rangesForOffsets first for only
  /// the visible offsets!
  if (sel->rangesAtLine(line, firstRangeIdx, lastRangeIdx))
  {
    ...
    for (int rangeIdx = firstRangeIdx; rangeIdx <= lastRangeIdx; ++rangeIdx)
    {
      ...
      QPainterPath path;
      path.addRoundedRect(startX, line * lineHeight + rect.top(), endX - startX,
                          rect.height(), 5, 5);
...
void TextEditorRenderer::renderLineSeparator(QPainter *painter, int line)
{
  ...
  if (config->useLineSeparator())
  {
    ...
    painter->drawLine(
        visibleRect.left(), y, visibleRect.right(),
        y); // draw from 0 to allow correct rendering of dotted lines
  }
  // PROF_END
}
...
void TextEditorRenderer::renderLineText(QPainter *painter, int line)
{
  ...
  //     renderer()->themeStyler()->getLineFormatRanges( line );
  QVector<QTextLayout::FormatRange> formats;
  // PROF_END
...
void TextEditorRenderer::renderCarets(QPainter *painter)
{
  ...
  if (renderer()->shouldRenderCaret())
  {
    ...
    for (int line = startLine; line <= endLine; ++line)
    {
      ...
      TextLayout *textLayout = renderer()->textLayoutForLine(line);
      for (int caret = 0, rangeCount = sel->rangeCount(); caret < rangeCount;
           ++caret)
      {
