...
TextEditorAutoCompleteComponent::TextEditorAutoCompleteComponent(
    TextEditorController *controller, TextEditorComponent *parent,
  ...
  , infoTipRef_(nullptr)
{
  ...
  p.setColor(QPalette::Highlight,
             p.color(QPalette::Highlight)); // prevents the non-focus gray color
  ...
  connect(listWidgetRef_, SIGNAL(itemClicked(QListWidgetItem *)), this,
          SLOT(listItemClicked(QListWidgetItem *)));
  connect(listWidgetRef_, SIGNAL(itemDoubleClicked(QListWidgetItem *)), this,
          SLOT(listItemDoubleClicked(QListWidgetItem *)));
  connect(listWidgetRef_,
          SIGNAL(currentItemChanged(QListWidgetItem *, QListWidgetItem *)),
          this, SLOT(showInfoTip()));
}
...
bool TextEditorAutoCompleteComponent::shouldDisplayAutoComplete(
    TextRange &range, QString &word)
{
  ...
  wordRange.expandToWord(doc, doc->config()->whitespaces(),
                         doc->config()->charGroups());
  wordRange.maxVar() = range.max(); // next go past the right caret!
  word = doc->textPart(wordRange.min(), wordRange.length())
             .trimmed(); // workaround for space select bug! #61
...
void TextEditorAutoCompleteComponent::showInfoTip()
{
  ...
  // for( QListWidgetItem *item : listWidgetRef_->findItems("*",
  // Qt::MatchWildcard)) {
  for (int i = 0; i < listWidgetRef_->count(); i++)
  {
    ...
    if (sDetail.contains(" = "))
    {
      ...
      int width
          = fm.horizontalAdvance(QString("%1   %2").arg(sLabel).arg(sType))
            + widthMod;
  ...
  int spacing = 0;
  menuRef_->resize(
      QSize(maxWidth + spacing + 2,
            (qMin(listWidgetRef_->count(), 10) * fm.height()) + 6));
  listWidgetRef_->resize(
      QSize(maxWidth + spacing,
            (qMin(listWidgetRef_->count(), 10) * fm.height() + 4)));
  //    menuRef_->resize(QSize(maxWidth + spacing + 2, (
  //    qMin(listWidgetRef_->count(), 10) * fm.height() ) ));
  //    listWidgetRef_->resize(QSize(maxWidth + spacing, (
  //    qMin(listWidgetRef_->count(), 10) * fm.height() )));
  ...
  QSize tipSize
      = infoTipRef_->tipText->documentLayout()->documentSize().toSize();
  ...
  QPoint newLoc(
      listWidgetRef_->parentWidget()->mapToGlobal(r.topRight()).x() + xOffset,
      listWidgetRef_->parentWidget()->mapToGlobal(r.topRight()).y() + 1);

  QRect screen = QApplication::primaryScreen()->availableGeometry();

  if (newLoc.x() + infoTipRef_->width() > screen.x() + screen.width()
      && (menuRef_->x() - infoTipRef_->width() - 1) >= 0)
  {
...
bool TextEditorAutoCompleteComponent::fillAutoCompleteList(
    TextDocument *document, const TextRange &range, const QString &word)
{
  listWidgetRef_->clear();
  QList<TextAutoCompleteItem *> items
      = document->autoCompleteProviderList()->findAutoCompleteItemsForRange(
          document, range, word);
  if (items.length() > 0)
  {
...
void TextEditorAutoCompleteComponent::positionWidgetForCaretOffset(int offset)
{
  ...
  int y = renderer->yPosForOffset(offset)
          - controller()
                ->textRenderer()
                ->viewportY(); // offset the y position based on how far
                               // scrolled down the editor is
  int x = renderer->xPosForOffset(offset)
          - controller()->textRenderer()->viewportX()
          + marginComponentRef_->widthHint()
          - 3; // adjusts x position based on scrollbars, line-number, and
               // position in line
  QPoint newLoc
      = editorComponentRef_->parentWidget()->parentWidget()->mapToGlobal(
          QPoint(x, y));

  // We want to constrain the list to only show within the available screen
  // space
  ...
  QRect screen = qApp->desktop()->availableGeometry(
      editorComponentRef_->parentWidget()->parentWidget());
#endif

  newLoc.setX(qMax(
      screen.left(),
      newLoc.x())); // constrain the origin of the list to the leftmost pixel
  newLoc.setY(qMax(
      screen.top(),
      newLoc.y())); // constrain the origin of the list to the topmost pixel
  newLoc.setX(qMin(screen.x() + screen.width() - menuRef_->width(),
                   newLoc.x())); // ensure that the entire width of the list can
                                 // be shown to the right
  if (newLoc.y() + menuRef_->height() > screen.bottom())
  { // if the list could go below the bottom, draw above
    newLoc.setY(qMin(newLoc.y(), screen.bottom())
                - menuRef_->height()); // positions the list above the word
  }
  else
  {
    newLoc.setY(
        newLoc.y()
        + renderer->lineHeight()); // places it below the line, as normal
  }
    ...
    switch (key->key())
    {
      ...
      case Qt::Key_Tab:
        if (listWidgetRef_->currentItem()
            && currentWord_ == listWidgetRef_->currentItem()->text())
        { // sends normal enter/return/tab if you've typed a full word
          menuRef_->close();
          QApplication::sendEvent(editorComponentRef_, event);
          return true;
        }
...
{
  listWidgetRef_->selectionModel()->select(modelIndex,
                                           QItemSelectionModel::SelectCurrent);
}

AutoCompleteDelegate::AutoCompleteDelegate(TextEditorController *controller,
                                           QObject *parent)
...
void AutoCompleteDelegate::paint(QPainter *painter,
                                 ...
                                 const QModelIndex &index) const
{
  ...
  typeRect.setX(nameRect.x() + nameRect.width() - fm.horizontalAdvance(sType)
                - 1);
...
QSize AutoCompleteDelegate::sizeHint(const QStyleOptionViewItem &,
...
void FakeToolTip::setText(const QString text)
{
  ...
  tipText->setDefaultStyleSheet(
      "p {color:" + themeRef_->foregroundColor().name() + "}");
