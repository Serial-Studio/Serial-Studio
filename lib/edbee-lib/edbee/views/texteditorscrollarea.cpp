...
void TextEditorScrollArea::layoutMarginWidgets()
{
  ...
  // set the margins
  //    int pixelFix = -1;  // removes the dirty little gray line between the
  //    components
  ...
  if (leftWidgetRef_)
  {
    QRect rect(0, top, left,
               qMin(leftWidgetRef_->maximumHeight(), viewport()->height() + 1));
  ...
  if (rightWidgetRef_)
  {
    QRect rect(viewport()->width() + left, top, right + 1,
               qMin(rightWidgetRef_->maximumHeight(), viewport()->height())
                   + 1);
  ...
  if (bottomWidgetRef_)
  {
    QRect rect(0, viewport()->height() + top,
               qMin(bottomWidgetRef_->maximumWidth(), viewport()->width()) + 1,
               bottom + 1);
