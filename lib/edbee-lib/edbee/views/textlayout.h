...
/// Modifying all cursor/positioning related methods, supporting single-cursor
/// items, while rendering multiple QTextLayout characters
///
...
public:
  ...
  void drawCursor(QPainter *painter, const QPointF &position,
                  int cursorPosition, int width) const;
  ...
  /// We assume the SOURCE character is 1... (TODO: store the orginal number of
  /// replace characters)
  qreal cursorToX(int cursorPos,
                  QTextLine::Edge edge = QTextLine::Leading) const;
  int xToCursor(qreal x, QTextLine::CursorPosition cpos
                         = QTextLine::CursorBetweenCharacters) const;

protected:
  QTextLayout *qtextLayout_;
  TextDocument *textDocumentRef_;
  TextRangeSet *singleCharRanges_; ///< A list textRanges_ used by TextLayout.
                                   ///< Every range in this list is treatet as a
                                   ///< single character for cusor-movement etc
...
{
public:
  TextLayoutBuilder(TextLayout *textLayout, QString &baseString,
                    QVector<QTextLayout::FormatRange> &baseFormatRanges);

  void replace(int index, int length, const QString replacement,
               QTextCharFormat format);

protected:
  TextLayout *textLayoutRef_; ///< A reference to the textlayout
  QString &baseString_; ///< A reference to the base-string. (This string is
                        ///< modified!)
  QVector<QTextLayout::FormatRange>
      &baseFormatRanges_; ///< A reference to the baseFormat range (This list is
                          ///< modified!)
};
