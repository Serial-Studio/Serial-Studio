...
/// Using control picutres makes it possible to show the control characters
/// (rquires a special font)
...
/// The init method is called if all objects required for editing have been
/// created!
...
int TextRenderer::lineHeight()
{
  ...
  return fm.ascent() + fm.descent() + fm.leading()
         + config()->extraLineSpacing(); // (the 1 is for the base line).
}
...
/// This method takes the maximum line length and multiplies it with the widest
/// character.
int TextRenderer::totalWidth()
{
  if (!totalWidthCache_)
  {
    for (int line = 0, cnt = textDocument()->lineCount(); line < cnt; ++line)
    {
      ...
      totalWidthCache_ = qMax(qRound(layout->boundingRect().right() + 0.5),
                              totalWidthCache_);
    }
  }
  ...
  if( !totalWidthCache_ ) {
      ...
      qlog_info() << "Maximum width:" << maxChars << " * " << fm.maxWidth() << "
  :: " << (fm.maxWidth() * maxChars); ttoalWidthCache_ = fm.maxWidth() *
  maxChars;
  }
  return totalWidthCache_;
  */
...
/// Often the M is to wide. That why we have a nr width which takes the 8 for
/// the width
...
static bool isControlCharacter(QChar charCode)
{
  ...
  if ((charCode >= QChar(0x202A) && charCode <= QChar(0x202E))
...
TextLayout *TextRenderer::textLayoutForLineForPlaceholder(int line)
{
  ...
  if (!textLayout)
  {
    ...
    int tabWidth
        = controllerRef_->widget()->fontMetrics().horizontalAdvance('M');
    ...
    {
      option.setFlags(
          QTextOption::ShowTabsAndSpaces); /// TODO: Make an option to show
                                           /// spaces and tabs
    }
    ...
    totalWidthCache_ = qMax(totalWidthCache_,
                            qRound(textLayout->boundingRect().width() + 0.5));
...
TextLayout *TextRenderer::textLayoutForLineNormal(int line)
{
  ...
  if (!textLayout)
  {
    ...
    QString text = doc->lineWithoutNewline(line);
    QVector<QTextLayout::FormatRange> formatRanges
        = themeStyler()->getLineFormatRanges(line);
    ...
    if (config()->renderBidiContolCharacters())
    {
      ...
      for (int i = 0; i < text.size(); ++i)
      {
        ...
        if (isControlCharacter(c))
        {
          ...
          // Better replacement for control character:
    ...
    for (int i = 0; i < text.size(); ++i)
    {
      ...
      {
        c = QChar(0x2400
                  + c.unicode()); // add     the Control Pictures range (see:
                                  // http://unicode.org/charts/PDF/U2400.pdf )
        text.replace(i, 1, c);
      }
    }
...
void TextRenderer::renderBegin(const QRect &rect)
{
  ...
  int calculatedEndLine = rawLineIndexForYpos(y + rect.height())
                          + 1; // add 1 line extra (for half visible lines)
  ...
  //    qlog_info() << ">> render startLine" << startLine_ << " t/m endLine=" <<
  //    endLine_  << "  ==> " << ( endLine_ - startLine_ ) << "
...
void TextRenderer::textDocumentChanged(edbee::TextDocument *oldDocument,
                                       edbee::TextDocument *newDocument)
{
  ...
  connect(newDocument, SIGNAL(textChanged(edbee::TextBufferChange, QString)),
          this, SLOT(textChanged(edbee::TextBufferChange, QString)));
  connect(newDocument, SIGNAL(lastScopedOffsetChanged(int, int)), this,
          SLOT(lastScopedOffsetChanged(int, int)));
}
...
{
  // qlog_info() << "**
  // lastScopedOffsetChanged("<<previousOffset<<","<<newOffset<<") **";
  Q_UNUSED(newOffset)
