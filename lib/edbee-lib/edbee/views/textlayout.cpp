...
void TextLayout::draw(QPainter *p, const QPointF &pos,
...
void TextLayout::drawCursor(QPainter *painter, const QPointF &position,
...
TextLayoutBuilder::TextLayoutBuilder(
    TextLayout *textLayout, QString &baseString,
...
void TextLayoutBuilder::replace(int index, int length,
                                const QString replacement,
                                QTextCharFormat format)
{
  baseString_.replace(index, length, replacement);
  textLayoutRef_->addSingleCharRange(
      index,
      replacement.length()); /// TODO: Should we store the original length!?!?!?
