...
//    qlog_info() << "[CACHE:xpos]" << offset << " >> " <<
//    xPosCache_.contains(offset);
...
void TextCaretCache::caretMovedFromOldOffsetToNewOffset(int oldOffset,
                                                        int newOffset)
{
  //    qlog_info() << "[CACHE:move]" << oldOffset<< " => " << newOffset << " <<
  //    " << xPosCache_.contains(oldOffset);
  Q_ASSERT(xPosCache_.contains(oldOffset));
  int xpos = xPosCache_.take(oldOffset);
  xPosCache_.insert(newOffset, xpos);

  //    qlog_info() << " CACHE: move " << oldOffset << " to " << newOffset << "
  //    => " << xpos;
}
