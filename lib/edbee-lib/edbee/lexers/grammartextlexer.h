...
private:
  ...
  QVector<MultiLineScopedTextRange *>
      closedMultiRangesRangesRefList_; ///< A list of all ranges (from other
                                       ///< lines) that have been closed. (only
                                       ///< valid during parsing)

  QVector<ScopedTextRange *>
      activeScopedRangesRefList_; ///< The current active scoped text ranges,
                                  ///< LINE (this is only valid during parsing)

  //    QVector<MultiLineScopedTextRange*> currentLineRangesList_;      ///< The
  //    current scope ranges (only valid during parsing)

  ScopedTextRangeList *lineRangeList_; ///< The scopes at current line (only
                                       ///< valid during parsing)
};
