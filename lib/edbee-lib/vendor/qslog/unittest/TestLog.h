...
public:
  ...
  QString type() const override { return mMyType; }
  ...
  int messageCount() const { return mMessages.count(); }
...
// Stores a "file system" inside a string list and can perform some operations
// on it.
...
public:
  ...
  const QStringList &filesList() { return files; }
...
protected:
  ...
  bool renameFileFromTo(const QString &from, const QString &to) override
  {
    ...
    if (editedFromPath.startsWith(fakePath)
        && editedToPath.startsWith(fakePath))
    {
