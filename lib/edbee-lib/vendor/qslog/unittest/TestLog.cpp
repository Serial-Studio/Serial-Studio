...
// Needed to convert from removeDestination return value to the type that we
// initially added.
...
// Autotests for QsLog.
// These tests are based on using a mock destination to check what was logged.
// Destinations are owned by the log, that's why the add/remove ping-pong is
// needed in the tests.
...
void TestLog::testAllLevels()
{
  ...
  mockDest
      = unique_cast(Logger::instance().removeDestination(destinationName1));
...
void TestLog::testLevelChanges()
{
  ...
  mockDest1
      = unique_cast(Logger::instance().removeDestination(destinationName1));
  mockDest2
      = unique_cast(Logger::instance().removeDestination(destinationName2));
...
void TestLog::testRotation()
{
  ...
  for (int i = 0; i < backupCount; ++i)
  {
    ...
    QCOMPARE(rotationStrategy.filesList().size(),
             1 + i + 1); // 1 log + "rotation count" backups
  }

  rotationStrategy.rotate();
  QCOMPARE(rotationStrategy.filesList().size(),
           backupCount + 1); // 1 log + backup count
}
...
void TestLog::testDestinationType()
{
  ...
  DestinationPtrU file = DestinationFactory::MakeFileDestination(
      "test.log", LogRotationOption::DisableLogRotation, MaxSizeBytes(5000),
      MaxOldLogCount(1));
  DestinationPtrU func
      = DestinationFactory::MakeFunctorDestination(&DummyLogFunction);

  QCOMPARE(
      Logger::instance().hasDestinationOfType(DebugOutputDestination::Type),
      false);
  QCOMPARE(Logger::instance().hasDestinationOfType(FileDestination::Type),
           false);
  QCOMPARE(Logger::instance().hasDestinationOfType(FunctorDestination::Type),
           false);
