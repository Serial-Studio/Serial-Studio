 ...
 * - io.driver.process.setOutputCapture - Set output capture (0=StdOut,1=StdErr,2=Both)
 *
 * Queries:
 * - io.driver.process.getRunningProcesses - List running processes for pipe picker
 * - io.driver.process.getConfiguration   - Get complete Process driver config
 */
class ProcessHandler {
public:
...
private:
  ...
  static CommandResponse setOutputCapture(const QString& id, const QJsonObject& params);
