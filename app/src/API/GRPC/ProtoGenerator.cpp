...
QString API::GRPC::ProtoGenerator::generateProto()
{
  ...
  // Shared response and streaming message types
  ...
  // Per-command request messages
  out << "// Per-command request messages\n\n";
  for (const auto& msg : std::as_const(message_defs))
    out << msg << "\n";

  // Service definition with typed RPCs and streaming endpoints
