...
The shared-table API is injected into the parser too: `tableGet(table, reg)` /
...
writes many variables per frame, resolve handles once in a top-level `local` and use
