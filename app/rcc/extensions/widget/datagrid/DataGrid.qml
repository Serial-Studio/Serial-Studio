...
Item {
  ...
  //
  // Column headers: the host publishes data, not chrome, so the package owns these strings
  //
  readonly property string titleHeader: qsTr("Title")
  ...
  //
  // Toolbar: Pause/Resume affordance (only when there's room). A package cannot import the
  // application's own QML components, so the strip is declared here; the visibility policy
  // (hidden when frozen or when the widget is too short) mirrors the built-in one exactly.
  //
  Item {
    id: toolbar

    readonly property bool shown: !root.frozen && root.height >= 220

    visible: toolbar.shown
    height: toolbar.shown ? 48 : 0
  ...
  Item {
    ...
    ScrollView {
      ...
      ListView {
        ...
        model: root.model ? root.model.datasets : null
        ...
        delegate: Rectangle {
          ...
          RowLayout {
            ...
            Label {
              ...
              text: rowItem.text.length > 0 ? rowItem.text : qsTr("Awaiting data…")
              leftPadding: 8
              rightPadding: 8
              elide: Label.ElideRight
              wrapMode: Text.NoWrap
              ...
              font: (Cpp_Misc_CommonFonts.widgetFontRevision,
                     Cpp_Misc_CommonFonts.widgetFont(root.uiScale))
              color: rowItem.text.length > 0
                     ? Cpp_ThemeManager.colors["table_text"]
                     : Cpp_ThemeManager.colors["placeholder_text"]
              opacity: rowItem.text.length > 0 ? 1.0 : 0.7

              HoverHandler {
                id: valueHover

                enabled: valueLabel.truncated && rowItem.text.length > 0
              }
              ToolTip.delay: 600
              ToolTip.text: rowItem.text
              ToolTip.visible: valueHover.hovered && valueLabel.truncated
                               && rowItem.text.length > 0
            }
