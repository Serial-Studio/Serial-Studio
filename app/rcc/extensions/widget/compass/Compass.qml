...
Item {
  ...
  //
  // Heading and its 16-wind label, computed here: the host publishes the dataset value
  // as-is, and wrapping it is this instrument's own convention
  //
  readonly property real heading: ((root.model.value % 360) + 360) % 360
  readonly property string cardinal: root.cardinalDirection(root.heading)

  function cardinalDirection(angle) {
    if ((angle >= 0 && angle < 22.5) || (angle >= 337.5 && angle <= 360))
      return qsTr("N")

    if (angle < 67.5)  return qsTr("NE")
    if (angle < 112.5) return qsTr("E")
    if (angle < 157.5) return qsTr("SE")
    if (angle < 202.5) return qsTr("S")
    if (angle < 247.5) return qsTr("SW")
    if (angle < 292.5) return qsTr("W")
    if (angle < 337.5) return qsTr("NW")
  ...
  // Active page is persisted per-widget through the model's config store.
  //
  SwipeView {
    id: swipeView
      ...
      Rectangle {
        ...
        Column {
          ...
          Text {
            ...
            font.family: Cpp_Misc_CommonFonts.monoFont.family
            text: root.getPaddedText(root.heading) + " "
                  + (root.cardinal.length < 2 ? root.cardinal + " "
                                              : root.cardinal)
          }
  ...
  Connections {
    ...
    function onCurrentIndexChanged() {
      ...
      root.model.setConfigValue("page", swipeView.currentIndex)
    }
  }
}
